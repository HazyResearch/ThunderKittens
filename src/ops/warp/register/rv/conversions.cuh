...
__device__ static inline int canonical_src_lane_dim1(int laneid, int x_or_y) {
    return (row*4)%32;
}
...
__device__ static inline void copy(RV1 &dst, const RV2 &src) {
    static_assert(T1::outer_dim == RV2::outer_dim, "Outer dimensions of the register vectors must be the same.");
    using D1 = RV1::dtype;
    using D2 = RV2::dtype;
    ...
    else static_assert(false, "Something has gone deeply wrong with how register vectors were instantiated.");
}
