...
template<typename T> concept st_type_naive_row_layout = (
    st_type<T> && st_naive_row_layout<typename T::layout>
);
...
template<detail::st_type_naive_row_layout ST, int num_blocks>
__host__ static inline void create_tensor_map(CUtensorMap *tma_map, bf16 *src) {
    ...
    constexpr CUtensorMapSwizzle      tma_swizzle     = (std::is_same_v<typename ST::layout, st_naive_row_0b_layout>) ? CU_TENSOR_MAP_SWIZZLE_NONE :
                                                        (std::is_same_v<typename ST::layout, st_naive_row_32b_layout>) ? CU_TENSOR_MAP_SWIZZLE_32B :
                                                        (std::is_same_v<typename ST::layout, st_naive_row_64b_layout>) ? CU_TENSOR_MAP_SWIZZLE_64B :
...
template<int height, int width, st_naive_row_layout L>
