...
// #define rt_bf_32x32 rt_bf<2, 2>
// #define st_bf_32x32 st_bf<2, 2>
// #define rt_fl_32x32 rt_fl<2, 2>
// #define st_fl_32x32 st_fl<2, 2>
// #define rt_cmplx_bf_32x32 rt_cmplx_bf<2, 2>
...
void fftconv_tk(
    ...
    bf16* o, int h, int n, int n1, int B_TILE, int H_TILE
){
    ...
    int do_print = threadIdx.x == 0 && blockIdx.x == 0;
    ...
    st_cmplx_bf_32x32 (&tw_mat)  = al.allocate<st_cmplx_bf_32x32>();

    // Registers; everyone loads
    rt_cmplx_bf_32x32 a_reg;       // Doing 32x32 tiles
    rt_cmplx_bf_32x32 a_tr;        // For transpositions of first matrix (but still in row-layout)
    ...
    for (int i = 0; i < H_TILE; i++) {
        ...
        for (int j = 0; j < B_TILE; j++) {
            ...
            // X = X * tw
            load(a_reg, tw_mat); // replace 
            kittens::mul(accum, accum, a_reg);

            // X = XF
            load(b_reg, fft_mat); // replace
            kittens::zero(mma_reg);
            ...
            // X = XFinv
            load(b_reg, fft_inv_mat); // replace
            kittens::zero(mma_reg);
            kittens::mma_AB(mma_reg, accum, b_reg, mma_reg);
            kittens::copy(accum, mma_reg);
