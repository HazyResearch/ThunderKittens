...
def benchmark_attention(configurations):
    ...
    for B, H, N, D, causal in configurations:
        ...
        q = torch.randn(B, N, H, D, dtype=torch.bfloat16, device='cuda:0').requires_grad_()
        k = torch.randn(B, N, H, D, dtype=torch.bfloat16, device='cuda:0').requires_grad_()
        v = torch.randn(B, N, H, D, dtype=torch.bfloat16, device='cuda:0').requires_grad_()
...
configurations = [
    ...
    (8,  16, 1024*2,  128, False),
    (4,  16, 1024*4,  128, False),
    (2,  16, 1024*8,  128, False),
    (1,  16, 1024*16, 128, False),
    # (16, 32, 768,    64,  False),
