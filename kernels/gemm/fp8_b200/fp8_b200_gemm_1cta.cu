...
constexpr int NUM_WORKERS = (NUM_CONSUMERS + NUM_PRODUCERS) * WARPGROUP_WARPS;
...
template<int SUPER_M=8> __device__ static inline int2 get_task_idx(const matmul_globals &g, int task_iter, bool is_consumer) {
    constexpr int M_BLOCK = 2*Mb, N_BLOCK = Nb;
    int task_id = task_iter * gridDim.x + blockIdx.x;
    int Rblocks = g.d.rows() / M_BLOCK, Cblocks = g.d.cols() / N_BLOCK;
    int super_rows = (Rblocks/SUPER_M)*SUPER_M,
        final_rows = Rblocks - super_rows,
        super_repeat = SUPER_M*Cblocks;
    if (task_id < super_rows * Cblocks) {
        return { 
            (SUPER_M*(task_id/super_repeat) + task_id%SUPER_M)*2 + is_consumer*(warpgroup::groupid()),
        ...
        return {
            (super_rows + remainder_id%final_rows)*2 + is_consumer*(warpgroup::groupid()),
...
void matmul(const __grid_constant__ matmul_globals g) {
    ...
    int warpid = ::warpid(), warpgroupid = warpgroup::groupid();
    ...
    if(warpgroupid == NUM_CONSUMERS) {
        ...
        else if((warpgroup::warpid() == 0 || warpgroup::warpid() == 1) && warp::laneid() == 0) { // launch the MMA's
