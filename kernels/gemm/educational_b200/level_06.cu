...
using b_tile = st_bf<TILE_N, TILE_K>;
using d_tile = st_bf<TILE_M, TILE_N / EPI_PIPE_DEPTH>;  // Sliced output tile
...
__global__ __launch_bounds__(NUM_THREADS, 1) void matmul_kernel(
    ...
    const __grid_constant__ d_gl D_layout, int M, int N, int K) {
  ...
  __shared__ semaphore inputs_arrived[PIPE_STAGES];  // TMA done -> MMA can read
  __shared__ semaphore inputs_finished[PIPE_STAGES]; // MMA done reading -> TMA can overwrite
  __shared__ semaphore compute_done;                 // All MMA done -> epilogue can start
  ...
  if (warpid == 0 && laneid == 0) {
    for (int iter_k = 0; iter_k < num_k_iters; iter_k++) {
      ...
      tma::load_async(b_smem[stage], B_layout, {bid_n, iter_k},
                      inputs_arrived[stage]);
    }
  }
    ...
    for (int iter_k = 0; iter_k < num_k_iters; iter_k++) {
      ...
      if (stage == PIPE_STAGES - 1)
        ...
        if (iter_k == 0) {
        mm_ABt(accum, a_smem[stage], b_smem[stage], inputs_finished[stage]);
      } else {
        mma_ABt(accum, a_smem[stage], b_smem[stage], inputs_finished[stage]);
      }
    }
...
void matmul(__nv_bfloat16 *A, __nv_bfloat16 *B, __nv_bfloat16 *C, int M, int N,
            int K) {
  a_gl A_layout{reinterpret_cast<bf16 *>(A), nullptr, nullptr, (unsigned long)M,
                (unsigned long)K};
  b_gl B_layout{reinterpret_cast<bf16 *>(B), nullptr, nullptr, (unsigned long)N,
                (unsigned long)K};
  d_gl D_layout{reinterpret_cast<bf16 *>(C), nullptr, nullptr, (unsigned long)M,
                (unsigned long)N};
