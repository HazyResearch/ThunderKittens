...
void matmul_kernel(
    ...
    for (int iter_k = 0; iter_k < num_k_iters; iter_k++) {
        if (threadIdx.x == 0) {
            constexpr int cp_size = sizeof(a_tile) + sizeof(b_tile);
            tma::expect_bytes(inputs_arrived, cp_size);
            tma::load_async(a_smem, A_layout, {bid_m, iter_k}, inputs_arrived);
            tma::load_async(b_smem, B_layout, {bid_n, iter_k}, inputs_arrived);
        }
        ...
        if (iter_k == 0) {
            warpgroup::mm_ABt(accum, a_smem, b_smem);
        } else {
            warpgroup::mma_ABt(accum, a_smem, b_smem);
        }
