...
using b_tile = st_bf<TILE_N / 2, TILE_K>;
    ...
    if (warpgroup::groupid() == NUM_CONSUMERS) {
        ...
        if (warp::laneid() == 0 && warpgroup::warpid() == 3) {
            ...
            for (int idx = 0; idx < iters_per_task; idx++) {
                ...
                if (idx == 0) mm2_ABt (accum, a_smem[input_ring][warpgroup::warpid()], b_smem[input_ring], inputs_finished[input_ring]);
                else          mma2_ABt(accum, a_smem[input_ring][warpgroup::warpid()], b_smem[input_ring], inputs_finished[input_ring]);
...
void matmul(__nv_bfloat16* A, __nv_bfloat16* B, __nv_bfloat16* C, int M, int N, int K) {
    ...
    b_gl B_layout{reinterpret_cast<bf16*>(B), nullptr, nullptr, (unsigned long)N, (unsigned long)K};
    ...
    matmul_kernel<<<grid, NUM_THREADS, smem_size>>>(A_layout, B_layout, D_layout, M, N, K);
}
