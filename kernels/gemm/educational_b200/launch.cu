...
int run_benchmark(size_t M, size_t N, size_t K) {
    ...
    float *h_B = new float[N * K]; // B stored transposed: (N x K)
    ...
    cudaMalloc(&d_B, N*K*sizeof(__nv_bfloat16));
    ...
    __nv_bfloat16 *h_B_bf16 = new __nv_bfloat16[N * K];
    for (int i = 0; i < M * K; ++i) h_A_bf16[i] = __float2bfloat16(h_A[i]);
    for (int i = 0; i < N * K; ++i) h_B_bf16[i] = __float2bfloat16(h_B[i]);
    cudaMemcpy(d_A, h_A_bf16, M*K*2, cudaMemcpyHostToDevice);
    cudaMemcpy(d_B, h_B_bf16, N*K*2, cudaMemcpyHostToDevice);
    std::cout << "Copied matrices to device" << std::endl;

    // Compute reference GEMM on GPU (B is transposed)
    ...
    std::cout << "Total count: " << int(M * N) << std::endl;
