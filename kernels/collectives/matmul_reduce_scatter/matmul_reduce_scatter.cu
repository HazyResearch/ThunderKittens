...
__device__ inline void kernel(const globals &G) {
    ...
    int num_blocks_per_col = globals::NUM_DEVICES * G.C.rows() / globals::ROW_BLOCK;
    ...
    if (warpgroup_id == config::NUM_WARPGROUPS - 1) {
        ...
        if (warp_id == 3 && lane_id == 0) {
            ...
            for (int task_idx = clusterIdx().x; task_idx < num_blocks; task_idx += gridDim.x / config::CLUSTER_SIZE) {
                ...
                int block_idx = (task_idx + (G.dev_idx + 1) * (num_blocks / globals::NUM_DEVICES)) % num_blocks;
            ...
            for (int task_idx = clusterIdx().x; task_idx < num_blocks; task_idx += gridDim.x / config::CLUSTER_SIZE) {
                ...
                int dst_dev_idx = row_block_idx / (num_blocks_per_col / globals::NUM_DEVICES);
                row_block_idx %= (num_blocks_per_col / globals::NUM_DEVICES);

                wait(C_arrived, get_phasebit<0>(phasebits, 0));
                update_phasebit<0>(phasebits, 0);

                tma::store_add_async(G.C[dst_dev_idx], outputs.C, {row_block_idx * 2 + ctarank, col_block_idx});
...
PYBIND11_MODULE(_C, m) {
    ...
    m.def("tk_matmul_reduce_scatter", &entrypoint);
}
