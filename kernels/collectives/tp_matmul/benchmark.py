...
from _C import TKParallelTensor, tk_tp_matmul


def nccl_tp_matmul_func(
    A: torch.Tensor,
    B: torch.Tensor,
    C: torch.Tensor,
) -> None:
...
def tk_tp_matmul_func(
    A: torch.Tensor,
    B: torch.Tensor,
    C: TKParallelTensor,
    barrier: TKParallelTensor,
) -> None:
    tk_tp_matmul(A, B, C, barrier)
...
def run(
    ...
    nccl_run = lambda: nccl_tp_matmul_func(A, B, C_nccl)
    tk_run = lambda: tk_tp_matmul_func(A, B, C_tk, barrier)

    if check_correctness:
        nccl_run()
        tk_run()
        check_diff("TP Matmul Diff Comparison", C_tk.data_, C_nccl)
...
if __name__ == "__main__":
    ...
    run(M, K, N, local_rank, local_world_size, check_correctness=False, do_profile=False)
