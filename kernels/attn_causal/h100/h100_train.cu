...
void attend_ker_fwd_train_causal(const int N, CUtensorMap* tma_q, CUtensorMap* tma_k, CUtensorMap* tma_v, CUtensorMap* tma_o, CUtensorMap* tma_l) {
    ...
    st_fl<4, 4, layout_o>::col_vec (&l_smem)   [NUM_WARPGROUPS] = al.allocate<st_fl<4, 4, layout_o>::col_vec, NUM_WARPGROUPS>();
    ...
    for (int qo_idx = blockIdx.x; qo_idx < qo_blocks; qo_idx++, tic ^= 1, toc ^= 1) {
        ...
        if (qo_idx + 1 < qo_blocks) {
            if (warpid == 0) {
                ...
                tma::set_bytes(vec_b, NUM_WARPGROUPS_BWD_QO * sizeof(float) * l_smem[0][0].length       * 2);
...
void attention_train_forward_causal(torch::Tensor q, torch::Tensor k, torch::Tensor v, torch::Tensor o, torch::Tensor l) {
    ...
    CUtensorMap* tma_l_d = tma::allocate_and_create_tensor_map<kittens::st_fl<4, 4, layout_q>::col_vec>(l_bf, (batch*heads*N)/(4 * 16));
