...
__global__ void dummy_kernel(
    const __grid_constant__ gl<fp4e2m1, 1, 1, 128, 128, st_fl4_e2m1<128, 128>> gmem
) {
    __shared__ st_fl4_e2m1<128, 128> smem;
    ...
    tma::expect(sem, smem);
    if(threadIdx.x == 0) {
        printf("semaphore expected\n");
    }
    __syncwarp();
    tma::load_async(smem, gmem, {}, sem);
    if(threadIdx.x == 0) {
        printf("load async\n");
    }
    __syncwarp();
    ...
    if(laneid() == 0) {
        ...
        for(int i = 0; i < 128; i++) {
            for(int j = 0; j < 128; j++) {
                printf("%d ", reinterpret_cast<uint8_t*>(&smem)[i*128+j]);
            }
            printf("\n");
        }
        printf("\n");
...
void inner_run(fp4e2m1 *d_A, size_t M, size_t N, size_t K, dim3 grid, dim3 block) {
    ...
    gl<fp4e2m1, 1, 1, 128, 128, st_fl4_e2m1<128, 128>> gmem{d_A, nullptr, nullptr, nullptr, nullptr};
    // std::cerr << "Launching kernel with grid (" << grid.x << ", " << grid.y << "), block (" << block.x << ")" << std::endl;
    dummy_kernel<<<1, 32>>>(gmem);
}
...
int run_benchmark(size_t M, size_t N, size_t K) {
    ...
    for (int i = 0; i < 128 * 128; ++i) h_A[i] = 1.0f;

    std::cout << "Initialized matrices" << std::endl;

    // Allocate device memory
    fp4e2m1 *d_A;
    cudaMalloc(&d_A, 128*128*sizeof(fp4e2m1));


    cudaMemset(d_A, 255, 128*128*sizeof(fp4e2m1));
    ...
    //     inner_run(d_A, M, N, K, grid, block);
    // }
