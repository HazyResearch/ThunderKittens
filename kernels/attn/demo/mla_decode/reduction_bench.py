"""
Usage:
    python tk_bench_reduction.py --grid_size 132 --sm_iters 4 --iterations 1000 --work_iters 4
...
def main():
    ...
    instructions = torch.zeros((1, grid, num_task_iters, 16), dtype=torch.int32, device="cpu")
    for i in range(grid):
        dst_seq = 0 
        uid = base_uid + i
    ...
    print(f"  Base SM count (grid_size): {args.grid_size}")
    print(f"  Reduction ops per SM (sm_iters): {args.sm_iters}")
    print(f"  Sequential reductions per instruction (num_iters): {args.num_iters}")
