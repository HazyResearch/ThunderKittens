...
void fwd_attend_ker(const __grid_constant__ fwd_globals<D> g) {
    ...
    if (threadIdx.x == 0) { 
        ...
        for(int j = 0; j < K::stages; j++) {
            ...
            init_semaphore(compute_done[j], 1, 0); 
        }
        ...
        tma::load_async(q_smem, g.q, q_tile_idx, qsmem_semaphore);
    ...
    if(warpgroupid == NUM_WARPGROUPS-1) {
        ...
        if constexpr (is_causal) {
            kv_iters = (seq_idx * (K::qo_height/kittens::TILE_ROW_DIM<bf16>)) - 1 + (1 * (K::qo_height/kittens::TILE_ROW_DIM<bf16>)); 
        ...
        for (auto kv_idx = 0; kv_idx <= kv_iters; kv_idx++) {
            ...
            if (warpid == 0) {
                mm_ABt(att_tm, q_smem, k_smem[(kv_idx)%K::stages], mma_semaphore);
            }
            ...
            if (warpid == 0) {
                mma_AB(o_tm, att_bf_tm, v_smem[(kv_idx)%K::stages], mma_semaphore);
            }
            // warpgroup::mma_async_wait();
