...
using global_layout   =  gl<int, 1, 1, -1, -1>;
using pglobal_layout  =  pgl<gl<int, 1, 1, -1, -1>, true>;
...
__global__ void all_reduce_int(kittens_pgl p_o) {
    if (threadIdx.x == 0 && threadIdx.y == 0 && threadIdx.z == 0 &&
        ...
        asm volatile(
            "multimem.red.relaxed.sys.global.add.v4.u32 [%0], {4,5,6,7};"
            :
            : "l"(p_o.mc_ptr)
            : "memory"
        );
...
int main() {
    ...
    int *host_mat_1 = new int[nelem];
    for (int i = 0; i < nelem; ++i) host_mat_1[i] = 1;

    int *host_mat_2 = new int[nelem];
    for (int i = 0; i < nelem; ++i) host_mat_2[i] = i;

    // Print data
    printf("Device 1: ");
    for (int i = 0; i < 10; ++i) {
        printf("%d ", host_mat_1[i]);
    }
    printf("... (%d elements)\n", nelem);

    printf("Device 2: ");
    for (int i = 0; i < 10; ++i) {
        printf("%d ", host_mat_2[i]);
    }
    printf("... (%d elements)\n", nelem);
    
    
    // Allocate and copy data to device
    int **dev_mats = new int*[NUM_DEVICES];
