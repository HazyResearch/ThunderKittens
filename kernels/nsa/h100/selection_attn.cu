...
template<ducks::st::all ST, ducks::rt::all RT>
__device__ static inline void warpgroup_load_t_split(RT &dst0, const ST &src, int width_offset){
    int local_warpid = warpid()%4;
    using T2 = RT::dtype;
    using U  = ST::dtype;
    using T  = base_types::packing<T2>::unpacked_type;
    ...
    int row =  (warp_laneid % 16);
    int col = (warp_laneid / 16) * 8 + local_warpid*16 + width_offset;
...
constexpr int BWD_CONSUMER_WARPGROUPS = (1); 
...
struct bwd_globals {
    ...
    using q_tile  =         st_bf<G::tile_h_qo, G::tile_width>; // 16*64*2*2=4096
    using k_tile  =         st_bf<G::tile_h,    G::tile_width>; // 64*64*2=8192
    using v_tile  =         st_bf<G::tile_h,    G::tile_width>; // 64*64*2=8192
    using og_tile =         st_bf<G::tile_h_qo, G::tile_width>; // 16*64*2*2=4096
    using qg_tile =         st_bf<G::tile_h_qo, G::tile_width>; // 16*64*2=2048
    using kg_tile =         st_bf<G::tile_h,    G::tile_width>; // 64*64*2=8192
    using vg_tile =         st_bf<G::tile_h,    G::tile_width>; // 64*64*2=8192
    using attn_tile =       st_bf<G::tile_h, G::tile_h_qo>; // 64*16*2=2048
    using l_tile  = col_vec<st_fl<G::tile_h_qo, G::tile_h>>; // 16*2*2 = 64
    using d_tile  = col_vec<st_fl<G::tile_h_qo, G::tile_h>>; // 16*2*2 = 64
...
causal_mask(auto &reg_tile, int qo_idx) {
    int k_blk = blockIdx.x * tile_h + kittens::warpid()*kittens::TILE_ROW_DIM<bf16>;
...
compute_bwd_loop(
        ...
        int qo_idx, int q_start, int tic, int toc, int stage_flag) 
{
    group<4>::sync(11); 
    ...
    warpgroup::store(ds_smem, ds_block_t_mma);
         ...
         auto &dst, auto &bar, int kv_head_idx, int toc) 
{
    ...
    if (kittens::warpid() % 4 == 0) {
        coord<kg_tile> tile_idx = {blockIdx.z, blockIdx.x, kv_head_idx, 0};
        tma::store_async<dim::DEPTH, cache_policy::NORMAL>(dst.kg, kg_smem[0], tile_idx);
    ...
    if (kittens::warpid() % 4 == 0) {
        coord<vg_tile> tile_idx = {blockIdx.z, blockIdx.x, kv_head_idx, 0};
        tma::store_async<dim::DEPTH, cache_policy::NORMAL>(dst.vg, vg_smem[0], tile_idx);
        tma::store_commit_group();
    }
    tma::store_async_wait(); 
}
...
void bwd_attend_ker(const __grid_constant__ bwd_globals<D> g) {
    ...
    vg_tile (*vg_smem)    = reinterpret_cast<vg_tile *>(&v_smem.data[0]); 
    ...
    int tic = 0, toc = 1;
    const int q_start = (is_causal) ? (blockIdx.x*G::tile_h) : (0);
    bool* mask_ptr = g.mask + blockIdx.z*g.mask_stride_b + blockIdx.y*g.mask_stride_h + blockIdx.x*g.k.depth();
    ...
    if (threadIdx.x == 0) {
        ...
        coord<k_tile> tile_idx_kv = {blockIdx.z, blockIdx.x, kv_head_idx, 0};
        tma::load_async<dim::DEPTH, cache_policy::NORMAL>(k_smem, g.k, tile_idx_kv, kv_b);
        tma::load_async<dim::DEPTH, cache_policy::NORMAL>(v_smem, g.v, tile_idx_kv, kv_b);

        for(int i=q_start; i < qo_blocks; i += 1) {
            if(mask_ptr[i]) {
                coord<q_tile> tile_idx_q = {blockIdx.z, i, blockIdx.y, 0};
                tma::expect_bytes(q_b[tic],   sizeof(q_smem[0]));
                tma::load_async<dim::ROW, cache_policy::NORMAL>(q_smem[tic],  g.q,  tile_idx_q, q_b[tic]);
                tma::expect_bytes(o_b[tic],   sizeof(og_smem[0]));
                tma::load_async<dim::ROW, cache_policy::NORMAL>(og_smem[tic], g.og, tile_idx_q, o_b[tic]);
                break;
            }
        }
        ...
        if (warpid % kittens::WARPGROUP_WARPS == 0) {
            ...
            for (int qo_idx = q_start; qo_idx < qo_blocks; qo_idx+=1) {
                if (qo_idx + 1 < qo_blocks && mask_ptr[qo_idx+1]) {
                    ...
                    wait(compute_done[tic], ((stage_flag - q_start)/(2))%2);
                    stage_flag += 1;
                    tic ^= 1;
                    toc ^= 1;
                    // Don't use tma::load_async for l_smem and d_smem because it sames have
            ...
            int stage_flag = q_start;
            for (auto qo_idx = q_start; qo_idx < qo_blocks; qo_idx++) {
                if(mask_ptr[qo_idx]) {
                    wait(compute_done[tic], ((stage_flag - q_start)/(2))%2);
                    stage_flag += 1;
            ...
            for (int qo_idx = q_start; qo_idx < qo_blocks; qo_idx++) {
                if(mask_ptr[qo_idx]) {
                    compute_bwd_loop<is_causal, G::tile_h_qo, G::tile_h, G::tile_width, D>(
                        ...
                        qo_idx, q_start, tic, toc, stage_flag
                    );
                    stage_flag += 1;
                    ...
                    if constexpr(G::num_k_chunk==1){
                        ...
                        warpgroup::mm_AtB(qg_reg[0], k_smem, ds_smem);
                        warpgroup::mma_commit_group(); 
                    } else{
                        rt_bf<16, 64> k_reg_chunk;
                        warpgroup_load_t_split(k_reg_chunk, k_smem, 0);
                        warpgroup::sync(warpgroupid+8);
                        warpgroup::mm_AB(qg_reg[0], k_reg_chunk, ds_smem);
                        warpgroup::mma_commit_group();
                        warpgroup::mma_async_wait();
                        warpgroup_load_t_split(k_reg_chunk, k_smem, 64);
                        warpgroup::sync(warpgroupid+8);
                        warpgroup::mm_AB(qg_reg[1], k_reg_chunk, ds_smem);
                        warpgroup::mma_commit_group();
                    }
                    wait(qg_ready, toc);
                   ...
                   bool causal)
{
    ...
    if (head_dim == 64)  {
        ...
        dim3 grid_bwd_2(host_ceil_div(seq_len, 4*kittens::TILE_ROW_DIM<bf16>), kv_heads, batch);
        threads = kittens::WARP_THREADS * BWD_NUM_WORKERS;
        mem_size = sizeof(bwd_global_args::v_tile) + sizeof(bwd_global_args::k_tile) + sizeof(bwd_global_args::attn_tile) + sizeof(bwd_global_args::qg_tile)
                + (sizeof(bwd_global_args::q_tile) + sizeof(bwd_global_args::og_tile) + sizeof(bwd_global_args::l_tile) + sizeof(bwd_global_args::d_tile))*2+4096;
