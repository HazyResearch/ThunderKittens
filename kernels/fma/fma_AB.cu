...
using ab_rv_vec = kittens::rv<bf16, 16>;
using c_rv_vec = kittens::rv<bf16, 64>;

template<ducks::rv::naive_layout RV, ducks::st::all ST>
__device__ static inline void load_shared_col_to_vec(RV &dst, const ST &src, int col_idx) {
    using U2 = ST::dtype; 
    using T2 = RV::dtype;
    ...
    for (auto w = 0; w < dst.outer_dim; ++w) {
        if (w < dst.outer_dim - 1 || dst.length %32 == 0 || laneid() < 16) {
            U tmp;
            move<T>::lds(tmp, src.idx(src_ptr, {laneid() + (w * 32), col_idx}));
            // printf("Loaded value %f from %d\n", __bfloat162float(tmp), laneid() + (w * 32));
...
__device__ static inline void add_val_to_col(RV &dst, T val, int col_idx) {
    ...
    for (auto w = 0; w < dst.outer_dim; ++w) {
        if (w < dst.outer_dim - 1 || dst.length %32 == 0 || laneid() < 16) {
            if (laneid() + (32 * w) == col_idx)
            {
                dst[w][0] += base_types::convertor<U, T>::convert(val);
            }
        }
    }
}
...
__device__ static inline void fma_AB(DST_RV &d, const SRC_RV &a, const ST &b_smem, const DST_RV &c, global_layout b_gl) {
    ...
    for (int i = 0; i < 64; ++i) 
    {
        load_shared_col_to_vec(b, b_smem, i);
        __syncwarp();
...
int main() {
    ...
    size_t a_size = 16 * sizeof(bf16);
    size_t b_size = 16 * 64 * sizeof(bf16);
    size_t c_size = 64 * sizeof(bf16);

    // Use float for host arrays and convert to/from bf16 during transfer
    float *host_mat_1_float = new float[16];
    // for (int i = 0; i < 16; ++i) host_mat_1_float[i] = 1.0f;
    // for (int i = 0; i < 16; ++i) host_mat_1_float[i] = float(i % 16);
    for (int i = 0; i < 16; ++i) host_mat_1_float[i] = dis(gen);
    ...
    float *host_mat_3_float = new float[64];
    for (int i = 0; i < 64; ++i) host_mat_3_float[i] = 0.0f;

    // Allocate host bf16 arrays for data transfer
    bf16 *host_mat_1 = new bf16[16];
    bf16 *host_mat_2 = new bf16[16 * 64];
    bf16 *host_mat_3 = new bf16[64];

    // Convert from float to bf16 for device transfer
    for (int i = 0; i < 16 * 64; ++i) {
        if (i < 16) host_mat_1[i] = __float2bfloat16(host_mat_1_float[i]);
    ...
    Compute Reference
    */
    float cpu_reference[64];
    for (int j = 0; j < 64; ++j) {
        float sum = 0.0f;        
        for (int i = 0; i < 16; ++i) {
            sum += host_mat_1_float[i] * host_mat_2_float[i * 64 + j];
        }
        cpu_reference[j] = sum;
    }
    ...
    global_layout a_gl(d_host_mat_1, nullptr, nullptr, 1, 16);
    global_layout b_gl(d_host_mat_2, nullptr, nullptr, 16, 64);
    global_layout c_gl(d_host_mat_3, nullptr, nullptr, 1, 64);
