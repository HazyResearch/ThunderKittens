...
template<typename Op> __device__ void run_op(const typename Op::config::globals &globals, persistent_state &ps) {
    static_assert(kernel_template<Op>, "lcsf kernel template parameter does not satisfy concept requirements");
    ...
    if constexpr (has_store<Op>) {
        ...
        if(warpid() >= NUM_CONSUMER_WARPS) { // code path for producer warps
            ...
            int num_iters = 0;
            common_setup_args<L> unif{common, num_iters, globals, **scratch_smem};
        ...
        // Put the scratch block at the very end of shared memory.
        scratch_alloc_block (*scratch_smem) = reinterpret_cast<scratch_alloc_block*>((uint64_t)(ps.shmem) + (MAX_BLOCK_SHARED_MEMORY - sizeof(scratch_alloc_block)));
        // Finish block will come right before the scratch block.
        finish_block  (*finish_smem) = reinterpret_cast<finish_block*>((((uint64_t)(scratch_smem) - sizeof(finish_block))/1024)*1024);
        static_assert(sizeof(scratch_alloc_block) + sizeof(finish_block) <= MAX_BLOCK_SHARED_MEMORY, "Finish block is too large for shared memory.");
