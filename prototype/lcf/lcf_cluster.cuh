...
template<typename lcft> concept cluster_kernel_template = requires {
...
void cluster_kernel(const __grid_constant__ typename lcft::layout::globals globals) {
    static_assert(cluster_kernel_template<lcft>, "lcf kernel template parameter does not satisfy concept requirements");
    ...
    common_state common;
    int num_iters = -1; // number of iterations for the current task
    int task_iter = -1; // which task are we on?
    common_setup_args<L> unif{common, task_iter, num_iters, globals, *scratch_smem};
    lcft::common_setup(unif, true);
    ...
    if(warpid() >= NUM_CONSUMER_WARPS) { // code path for producer warps
        ...
        for(task_iter = 0; true; task_iter++) {
            ...
            lcft::consumer::setup({c_state, unif}); // setup consumer state, true means we are not prepared, so we need to do the setup
#ifdef CONSUMER_UNROLL
            #pragma unroll CONSUMER_UNROLL_VALUE
#endif
            for(int it = 0; it < num_iters; it++) {
                lcft::consumer::setup({c_state, unif}, false, it); // setup consumer state for the current iteration
