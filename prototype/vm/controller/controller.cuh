...
template<typename config, typename globals, typename... ops> __device__ void main_loop(const globals &g, ::kittens::prototype::vm::state<config> &kvms) {
         ...
         kvms.instruction_index++, kvms.instruction_ring = ring_advance<config::INSTRUCTION_PIPELINE_STAGES>(kvms.instruction_ring))
    {
        ...
        // Step 0. Wait for the previous instruction to complete & invalidate its semaphores
        ...
        int last_opcode = kvms.all_instructions[last_instruction_ring].instructions[0];
        for (int i = 0; i < config::NUM_PAGES; i++) 
        {
            if (kvms.instruction_index == 0) kvms.pid_order()[i] = i;
            else
            {
