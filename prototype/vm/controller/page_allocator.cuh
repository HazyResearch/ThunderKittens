...
template<typename config, int PAGE_COUNT> __device__ static inline void try_assign(::kittens::prototype::vm::state<config> &kvms, semaphore *arrived, semaphore *finished, int (&assignment)[config::PAGE_RING_SIZE], int assignment_ring, uint32_t assignment_counter, int &local_assignment_counter, uint32_t shift_mask, int &expected_phase) {
    constexpr bool is_mini_page = PAGE_COUNT == config::NUM_MINI_PAGES;
    ...
    asm volatile(
        ...
        : "r"(mbar_ptr), "r"(expected_phase), "r"(membermask)
    );
// #ifdef KVM_DEBUG
...
template<typename config, typename globals, int end_thread, typename... ops> __device__ void inline page_allocator_loop(const globals &g, ::kittens::prototype::vm::state<config> &kvms) {
    ...
    int page_phase = 1, mini_page_phase = 1;
