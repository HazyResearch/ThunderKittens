...
struct test_dsmem_vec { // load with dsmem, write out normally
    ...
    __device__ static void device_func(const GL &input, const GL &output) {
        ...
        if(threadIdx.x == 0) kittens::tma::expect(dsmem_semaphore, dst_vec);

        kittens::everyone::tma::cluster::sync();

        if(threadIdx.x == 0) kittens::tma::cluster::store_async(dst_vec, src_vec, (blockIdx.x+3)%4, dsmem_semaphore);
...
void thread::memory::vec::dsmem::tests(test_data &results) {
    std::cout << " ----- Starting ops/thread/memory/vec/dsmem tests! -----\n" << std::endl;
