...
struct test_dsmem { // load with dsmem, write out normally
    ...
    __device__ static void device_func(const GL input, const GL output) {
        ...
        if(threadIdx.x == 0) kittens::tma::expect(dsmem_semaphore, dst_tile);
        kittens::everyone::tma::cluster::sync();
        if(threadIdx.x == 0) kittens::tma::cluster::store_async(dst_tile, src_tile, (blockIdx.x+3)%4, dsmem_semaphore);
...
struct dsmem_wrapper_2d {
    ...
    static void run(test_data& results) {
        ...
        if constexpr (test::template valid<H, W, NUM_WORKERS, args...>::value) {
            ...
            dsmem_global_wrapper_2d<test, dtype, H, W, NUM_WORKERS, GL, args...><<<4, NUM_WORKERS*32, kittens::MAX_SHARED_MEMORY-1024>>>(input, output);
...
void thread::memory::tile::dsmem::tests(test_data &results) {
    std::cout << " ----- Starting ops/thread/memory/tile/dsmem tests! -----\n" << std::endl;
