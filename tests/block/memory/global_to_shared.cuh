...
struct load_store {
    ...
    static inline const std::string test_identifier = "block_shared_loadstore";
    ...
    template<int H, int W, int NW, kittens::ducks::st_layout::all L> __device__ static void device_func(const kittens::bf16 *input, kittens::bf16 *output) {
        ...
        kittens::block<NW>::load(shared_tile, input, W*16);
        kittens::block<NW>::store(output, shared_tile, W*16);
    }
};
struct load_store_async {
    template<int H, int W, int NW, kittens::ducks::st_layout::all L> using valid = std::bool_constant<NW == 8 &&
        (!std::is_same_v<L, kittens::ducks::st_layout::tma_swizzle> || W == 1 || W == 2 || W == 4)>;
    static inline const std::string test_identifier = "block_shared_loadstore_async";
    template<int H, int W, int NW, kittens::ducks::st_layout::all L> __host__ static void host_func(const std::vector<float> &i_ref, std::vector<float> &o_ref) {
        o_ref = i_ref; // overwrite the whole thing
    }
    template<int H, int W, int NW, kittens::ducks::st_layout::all L> __device__ static void device_func(const kittens::bf16 *input, kittens::bf16 *output) {
        ...
        block.sync();
        kittens::block<NW>::load_async(shared_tile, input, W*16, barrier);
        barrier.arrive_and_wait();

        kittens::block<NW>::store_async(output, shared_tile, W*16, barrier);
        barrier.arrive_and_wait();
    }
};

struct vec_load_store {
    template<int S, int NW> using valid = std::bool_constant<NW == 8 && S<=64>;
    static inline const std::string test_identifier = "block_shared_vec_loadstore";
    ...
    template<int S, int NW> __device__ static void device_func(const kittens::bf16 *input, kittens::bf16 *output) {
        ...
        kittens::block<NW>::load(shared_vec, input);
        kittens::block<NW>::store(output, shared_vec);
    }
};
