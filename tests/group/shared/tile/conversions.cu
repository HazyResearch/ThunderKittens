...
struct test_swap_layout {
    template<int H, int W, int NW, kittens::ducks::st_layout::all L1, kittens::ducks::st_layout::all L2> using valid = std::bool_constant<H%NW==0 && W*H<=64 &&
        (!(std::is_same_v<L1, kittens::ducks::st_layout::tma_swizzle> || std::is_same_v<L2, kittens::ducks::st_layout::tma_swizzle>) || W == 1 || W == 2 || W == 4)>; // this is group-level
...
void group::shared::tile::conversions::tests(test_data &results) {
    ...
    sweep_st_layout_size_2d<test_swap_layout, SIZE, SIZE, 2, kittens::ducks::st_layout::tma_swizzle>::run(results);
    ...
    if constexpr (TEST_INTENSITY > 1) {
        ...
        sweep_st_layout_size_2d<test_swap_layout, SIZE, SIZE, 4, kittens::ducks::st_layout::tma_swizzle>::run(results);
        ...
        if constexpr (TEST_INTENSITY > 3) {
            ...
            sweep_st_layout_size_2d<test_swap_layout, 12, 4, 12, kittens::ducks::st_layout::tma_swizzle>::run(results);
