...
struct pseudo_shared_test {
    ...
    static inline const std::string test_identifier = [] {
        return std::string("pseudo_warpgroup_") + (use_mma ? "mma_" : "mm_") + op_name(trans_a, trans_b) + "_fp32_fp16";
    }();
...
struct pseudo_reg_shared_test {
    ...
    static inline const std::string test_identifier = [] {
        return std::string("pseudo_warpgroup_reg_") + (use_mma ? "mma_" : "mm_") + op_name(kittens::transpose::N, trans_b) + "_fp32_fp16";
    }();
...
void group::mma::warpgroup::pseudo_fp32_fp16::tests(test_data &results) {
    std::cout << " ----- Starting ops/mma/warpgroup/pseudo_fp32_fp16 tests! -----\n" << std::endl;
