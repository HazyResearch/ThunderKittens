...
struct pseudo_shared_test {
    ...
    static inline const std::string test_identifier = [] {
        return std::string("pseudo_warpgroup_") + (use_mma ? "mma_" : "mm_") + op_name(trans_a, trans_b) + "_fp16_fp16";
    }();
    ...
    __device__ static void device_func(const GTL_A &a_input, const GTL_B &b_input, GTL_C &c_output) {
        ...
        using A_ST = std::conditional_t<
            ...
            kittens::st_hf<16*H, 16*K>,
            kittens::st_hf<16*K, 16*H>
        >;
        using B_ST = std::conditional_t<
            trans_b == kittens::transpose::N,
            kittens::st_hf<16*K, 16*W>,
            kittens::st_hf<16*W, 16*K>
        >;
    ...
    using make_a_layout = typename std::conditional_t<
        trans_a == kittens::transpose::N,
        kittens::gl<kittens::half, 1, 1, 16*H, 16*K::value>,
        kittens::gl<kittens::half, 1, 1, 16*K::value, 16*H>
    >;
    template<int H, int W, typename K>
    using make_b_layout = typename std::conditional_t<
        trans_b == kittens::transpose::N,
        kittens::gl<kittens::half, 1, 1, 16*K::value, 16*W>,
        kittens::gl<kittens::half, 1, 1, 16*W, 16*K::value>
    >;
    template<int H, int W, typename K>
    using make_c_layout = typename kittens::gl<kittens::half, 1, 1, 16*H, 16*W>;
};
...
struct pseudo_reg_shared_test {
    ...
    static inline const std::string test_identifier = [] {
        return std::string("pseudo_warpgroup_reg_") + (use_mma ? "mma_" : "mm_") + op_name(kittens::transpose::N, trans_b) + "_fp16_fp16";
    }();
    ...
    __device__ static void device_func(const GTL_A &a_input, const GTL_B &b_input, GTL_C &c_output) {
        constexpr int K = _K::value;
        using A_ST = kittens::st_hf<16*H, 16*K>;
        using B_ST = std::conditional_t<
            trans_b == kittens::transpose::N,
            kittens::st_hf<16*K, 16*W>,
            kittens::st_hf<16*W, 16*K>
    ...
    template<int H, int W, typename K>
    using make_a_layout = typename kittens::gl<kittens::half, 1, 1, 16*H, 16*K::value>;
    template<int H, int W, typename K>
    using make_b_layout = typename std::conditional_t<
        trans_b == kittens::transpose::N,
        kittens::gl<kittens::half, 1, 1, 16*K::value, 16*W>,
    ...
    static void run(test_data& results) {
        ...
        if constexpr (test::template valid<H, W, NUM_WORKERS, _K, args...>::value) {
            ...
            cudaFuncSetAttribute(
                pseudo_mma_global_wrapper_2d<test, kittens::half, H, W, NUM_WORKERS, GTL_A, GTL_B, GTL_C, _K, args...>,
...
void group::mma::warpgroup::pseudo_fp16_fp16::tests(test_data &results) {
    std::cout << " ----- Starting ops/mma/warpgroup/pseudo_fp16_fp16 tests! -----\n" << std::endl;
