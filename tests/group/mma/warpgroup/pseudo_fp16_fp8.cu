...
#ifdef TEST_GROUP_MMA_WARPGROUP_PSEUDO_FP16_FP8

namespace {

using D_dtype = kittens::half;
...
struct pseudo_shared_abt_test {
    ...
    static inline const std::string test_identifier =
        std::string("pseudo_warpgroup_") + (use_mma ? "mma_" : "mm_") + "ABt_fp16_" + fp8_name<T_AB>();
    ...
    __host__ static void host_func(const std::vector<float> &i_ref, std::vector<float> &o_ref) {
        ...
        constexpr int R_CHUNK = kittens::TILE_COL_DIM<AB_dtype>;
        constexpr int b_offset = H * K * 256;

        for(int i = 0; i < M; i++) {
            for(int j = 0; j < N; j++) {
                kittens::half accum = __float2half(0.f);
                for(int kk = 0; kk < R; kk += R_CHUNK) {
                    float sum = __half2float(accum);
                    for(int k = kk; k < kk + R_CHUNK; k++) {
                        sum += i_ref[i * R + k] * i_ref[b_offset + j * R + k];
                    }
                    accum = __float2half(sum);
                }
                o_ref[i * N + j] = __half2float(accum);
            }
        }
    }
...
struct pseudo_reg_shared_abt_test {
    ...
    static inline const std::string test_identifier =
        std::string("pseudo_warpgroup_reg_") + (use_mma ? "mma_" : "mm_") + "ABt_fp16_" + fp8_name<T_AB>();
...
void group::mma::warpgroup::pseudo_fp16_fp8::tests(test_data &results) {
    std::cout << " ----- Starting ops/mma/warpgroup/pseudo_fp16_fp8 tests! -----\n" << std::endl;
