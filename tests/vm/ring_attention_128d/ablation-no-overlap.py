            ...
            for head_idx in range(H):
                for qo_idx in range(num_qo_blocks):
                    for __cta_rank in range(2):
                        dev_instructions[instruction_idx%SM_COUNT].append(
...
for ring_stage in range(num_ring_stages):
    ...
    timings = []
    for torch_device in torch_devices:
        instructions.append(generate_instructions(False, ring_stage, torch_device))
    ...
    if ring_stage < num_ring_stages - 1:
        instructions = []
        timings = []
        for torch_device in torch_devices:
            instructions.append(generate_instructions(True, ring_stage, torch_device))
            timings.append(torch.zeros((SM_COUNT, instructions[torch_device.index].shape[1], 128), dtype=torch.int32, device=torch_device))
        all_instructions.append(instructions)
        all_timings.append(timings)
...
###
print('\nLaunching kernel...')
assert len(all_instructions) == len(all_timings) == NUM_DEVICES*2-1
...
for i in range(len(all_instructions)):
    ring_attention(
        all_instructions[i], barriers, all_timings[i],
        Qs, K0s, K1s, V0s, V1s, Os, Ls, Ms
    )
    for dev_id in dev_ids:
        torch.cuda.synchronize(dev_id)
...
for i in range(NUM_WARMUPS):
    ...
    for j in range(len(all_instructions)):
        ring_attention(
            all_instructions[j], barriers, all_timings[j],
            Qs, K0s, K1s, V0s, V1s, Os, Ls, Ms
        )
