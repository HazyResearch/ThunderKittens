...
def save_gantt_chart(Timings, Instructions, save_all=False, name=None, verbose=False, clock_rate=1800):
    ...
    timing_events = {
        80: {'marker': 6, 'color': 'black', 'label': 'Load weights', 'size': 40, 'alpha': 1},
        81: {'marker': 6, 'color': 'blue', 'label': 'Load activations', 'size': 40, 'alpha': 1},
        82: {'marker': 6, 'color': 'red', 'label': 'Store outputs', 'size': 40, 'alpha': 1},
        83: {'marker': 7, 'color': 'black', 'label': 'Load weights', 'size': 40, 'alpha': 1},
        84: {'marker': 7, 'color': 'blue', 'label': 'Load activations', 'size': 40, 'alpha': 1},
        85: {'marker': 7, 'color': 'red', 'label': 'Store outputs', 'size': 40, 'alpha': 1},
    }
    ...
    for proc in range(Timings.shape[0]):
        for instr in range(Timings.shape[1]):
            ...
            if Timings[proc, instr, 4].item() > 0:
                        ...
                        for event_id, event_props in timing_events.items():
                            if Timings[proc, instr, event_id].item() > 0:
                                ...
                                ax.scatter(event_time, proc, 
                                        ...
                                        s=event_props['size'],
                                        alpha=event_props['alpha'],
                                        zorder=5)  # Ensure markers are on top

    # Customize the chart
    ax.set_xlabel('Time (us)')
    ax.set_ylabel('SM')
    ax.set_title('MLP Execution Timeline')
    ...
    if save_all:
        marker_legend_elements = [plt.Line2D([0], [0], marker=props['marker'], color='w', 
                                 ...
                                 label=props['label'], alpha=props['alpha'])
