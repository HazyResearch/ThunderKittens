...
struct globals {
    ...
    using barrier_layout = gl<uint, 1, 16, 6, 48>; // (num_layers, num_ops, qkv_heads)
    ...
    activation_layout hidden_states;
    rms_scale_layout attn_ln_weight;
    proj_weight_layout qkv_proj;
    rope_table_layout rope_cos;
    rope_table_layout rope_sin;
    activation_layout post_ln_rope_q;
...
struct RMS_MatVec_Rope_Append_Op {
    ...
    struct loader {
        static __device__ void run(const globals &g, state<config> &s) {
            ...
            if (laneid() < 4) {
                ...
                tma::load_async(weight_chunk, g.qkv_proj, {inst.layer_idx, inst.qkv_block_idx, laneid()}, weights_arrived(s, laneid()));
            } else if (laneid() == 4) {
                // Activation
                ...
                tma::load_async(rms_scale, g.attn_ln_weight, {inst.layer_idx, 0}, rms_scale_arrived(s));
            } else if (laneid() == 6) {
                // Rope cos
...
PYBIND11_MODULE(rms_matvec_rope_append, m) {
    m.doc() = "";
    kittens::py::bind_kernel<kvm<config, globals, RMS_MatVec_Rope_Append_Op<config>>>(m, "rms_matvec_rope_append",
        &globals::instructions,
        &globals::barriers,
        &globals::timings,
        &globals::hidden_states,
        &globals::attn_ln_weight,
        &globals::qkv_proj,
        &globals::rope_cos,
        &globals::rope_sin,
