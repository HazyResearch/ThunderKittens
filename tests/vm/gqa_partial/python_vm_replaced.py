...
TIMING_WIDTH = 128
_instructions = [[[0] * INSTRUCTION_WIDTH] for _ in range(148)]
timings = torch.zeros((NUM_BLOCKS, 1, TIMING_WIDTH), dtype=torch.int32).to(device=7)

def partial_attention(globals: Globals, instruction: PartialAttention):

    _instructions[0][0] = [1, instruction.layer_idx, instruction.kv_head_idx, instruction.num_partials, instruction.partial_idx] + [0] * (INSTRUCTION_WIDTH - 5)
    instructions = torch.tensor(_instructions, dtype=torch.int32).to(device=7)

    gqa_partial(
        instructions, 
        globals.timings, 
        globals.post_ln_rope_q.reshape(32, 64),
        globals.k_cache,
        globals.v_cache,
