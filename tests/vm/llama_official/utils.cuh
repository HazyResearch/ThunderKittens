...
__device__ static inline void matvec_reduce(uint8_t *scratch, rv_t &sum_vec) {
  ...
  for (int i = 0; i < Config::NUM_CONSUMER_WARPS; i++) {
    ...
    // TODO: for now, deliberately not using sizeof(sv_t) here because we've had
...
old_rms_norm(Globals &g, state<Config> &s, rv_t &activations_vec,
             int rms_scale_activation_page, semaphore &activations_arrived,
             semaphore &rms_scale_arrived, int scratch_offset) {
  ...
  sv_bf<REDUCTION_DIM_PER_WARP> *rms_scale_smem =
      reinterpret_cast<sv_bf<REDUCTION_DIM_PER_WARP> *>(
          s.pages[rms_scale_activation_page].ptr());
  sv_bf<REDUCTION_DIM_PER_WARP> *activations_smem =
      reinterpret_cast<sv_bf<REDUCTION_DIM_PER_WARP> *>(
          s.pages[rms_scale_activation_page].ptr(sizeof(sv_bf<2048>)));
...
template <typename rt_t, int WARPS_PER_PAGE, typename Config, typename Globals,
          typename rv_t>
__device__ inline void
old_matvec(Globals &g, state<Config> &s, rv_t &activations_vec,
           semaphore &weights_arrived, int weight_pid, int scratch_offset) {
  ...
  st_slice_t(&weights_smem)[WARPS_PER_PAGE] =
      reinterpret_cast<st_slice_t(&)[WARPS_PER_PAGE]>(s.pages[weight_pid]);
