...
template <typename config, typename globals> struct attention_partial {
    ...
    __device__ static inline semaphore &K_finished(state<config> &s,
                                                   int stage) {
        return s.semaphores()[3 + NUM_STAGES * 2 + stage * 2];
    }
    __device__ static inline semaphore &V_finished(state<config> &s,
                                                   int stage) {
...
#pragma unroll
    ...
    struct consumer {
        static __device__ void run(const globals &g, state<config> &s) {
            ...
            if (warpid() == 0) {
                ...
                // sv_bf<256> &true_qsmem = *reinterpret_cast<sv_bf<256>
                // *>(&Q_smem);
                ...
                for (int i = 0; i + start_blk_idx < end_blk_idx; ++i) {
                    ...
                    warp::sub(diff_scaled_max_vec_reg, last_scaled_max_vec_reg,
                              scaled_max_vec_reg);
                    warp::exp2(diff_scaled_max_vec_reg,
                    ...
                    warp::mul(norm_vec_reg, norm_vec_reg,
                              diff_scaled_max_vec_reg);
    ...
    struct storer {
        ...
        static inline __device__ void
        store_o_no_skip(const globals &g, state<config> &s,
