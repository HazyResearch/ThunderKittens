...
using weights = gl<bf16, 1, -1, -1, globals::hidden_dim, st_bf<globals::matvec_block_size, 512>>; // assumed to be N by 2048 (X@W.T).
using activations = gl<bf16, 1, 1, 1, globals::hidden_dim, sv_bf<globals::hidden_dim>,sv_bf<globals::matvec_block_size>>;
using barriers = gl<bf16, 1, -1, 6, globals::head_dim>; 
    ...
    struct loader
    {
        static __device__ void run(const globals &g, state<config> &s)
        {
            ...
            else if (laneid() == PAGE_RMS_SCALE)
            {
                ...
                tma::load_async(rms_scale, g.mlp_ln_weights, {}, rms_scale_arrived(s));
            }
