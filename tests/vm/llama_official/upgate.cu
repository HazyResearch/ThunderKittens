...
template <typename Config, typename Globals> struct rms_upgate_silu {
  ...
  struct pipeline_specifics {
    static __device__ inline void gmem_wait(const Globals &g,
      ...
      while (*(volatile int *)&g.Bar[{inst.layer_idx, prev_opcode - 1, 0}] <
    ...
    static __device__ inline void load_iter(state<Config> &s, const globals &g,
      ...
      if (iter % 2 == 0) {
        tma::load_async<dim::ROW, cache_policy::EVICT_FIRST>(
            weight_chunk, g.up_weights, {inst.layer_idx, block_idx, col_idx},
            sem);
      } else {
        tma::load_async<dim::ROW, cache_policy::EVICT_FIRST>(
            weight_chunk, g.gate_weights, {inst.layer_idx, block_idx, col_idx},
            sem);
      }
    }
    ...
    static __device__ inline void store(state<Config> &s, const Globals &g,
      ...
      matvec_reduce<Config, sv_fl<16>, rv_fl<16>,
                    pipeline::SCRATCH_BYTES_PER_WARP>(prev_output_scratch_start,
                                                      up_out);
      matvec_reduce<Config, sv_fl<16>, rv_fl<16>,
      ...
      if (laneid() == 0) {
        tma::store_async<cache_policy::EVICT_LAST>(g.silu_out, out_smem,
                                                   {block_idx});
