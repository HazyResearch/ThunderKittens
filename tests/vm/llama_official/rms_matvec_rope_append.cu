...
template <typename Config, typename Globals> struct rms_qkv_rope_append {
  static constexpr int opcode =
      OPCODE_RMS_QKV_MatVecRopeAppend; // Op index within the layer -- controls
  ...
  __device__ static inline uint8_t *get_rope_cos_ptr(state<Config> &s) {
    return (uint8_t *)s.scratch() + 512;
  }
  __device__ static inline uint8_t *get_rope_sin_ptr(state<Config> &s) {
    return (uint8_t *)s.scratch() + 512 + 256;
  }
  __device__ static inline rope_t &get_rope_cos(state<Config> &s) {
    return *reinterpret_cast<rope_t *>(get_rope_cos_ptr(s));
  }
  __device__ static inline rope_t &get_rope_sin(state<Config> &s) {
  ...
  struct pipeline_specifics {
    ...
    static __device__ inline void gmem_wait(const Globals &g,
      ...
      if (inst.layer_idx > 0) {
        while (*(volatile int *)&g
                    .Bar[{inst.layer_idx - 1, OPCODE_DownProjResidual - 1, 0}] <
    ...
    static __device__ inline void load_iter(state<Config> &s, const globals &g,
      ...
      tma::load_async<dim::ROW, cache_policy::EVICT_FIRST>(
          weight_chunk, g.qkv_weights, {inst.layer_idx, block_idx, col_idx},
          sem);
    }
    ...
    static __device__ inline void store(state<Config> &s, const Globals &g,
      ...
      sv_fl<16> &qkv_proj_smem = *reinterpret_cast<sv_fl<16> *>(
          (float *)s.scratch() + (32 * output_stage));
      sv_bf<16> &qkv_proj_smem_bf = *reinterpret_cast<sv_bf<16> *>(
          (float *)s.scratch() + (32 * output_stage));

      rv_fl<16> qkv_proj, rope_cos, rope_sin;

      // warp::load(rope_cos, g.rope_cos, {0, 0, static_cast<int>(g.pos_id),
      // block_idx % 4}); warp::load(rope_sin, g.rope_sin, {0, 0,
      // static_cast<int>(g.pos_id), block_idx % 4});
      ...
      sv_fl<16> &rope_cos_sv =
          *reinterpret_cast<sv_fl<16> *>(get_rope_cos_ptr(s) + head_chunk * 64);
      sv_fl<16> &rope_sin_sv =
          *reinterpret_cast<sv_fl<16> *>(get_rope_sin_ptr(s) + head_chunk * 64);

      // sv_fl<16> &rope_cos_sv = reinterpret_cast<sv_fl<16>
      // *>(&(get_rope_cos(s)[0]))[block_idx % 4]; sv_fl<16> &rope_sin_sv =
      // reinterpret_cast<sv_fl<16> *>(&(get_rope_sin(s)[0]))[block_idx % 4];
      ...
      if (laneid() == 0) {
        ...
        if (block_idx < K_BLK_START) { // Q
          ...
          int dim_idx =
              (base_index % Globals::head_dim) / Globals::matvec_block_size;
          tma::store_async<cache_policy::EVICT_LAST>(
    ...
    static __device__ void run(const Globals &g, state<Config> &s) {
      ...
      if (laneid() == 0) {
        ...
        tma::load_async<cache_policy::EVICT_LAST>(
            rope_cos, g.rope_cos, {0, 0, static_cast<int>(g.pos_id), 0}, sem);
        tma::load_async<cache_policy::EVICT_LAST>(
            rope_sin, g.rope_sin, {0, 0, static_cast<int>(g.pos_id), 0}, sem);
      }
