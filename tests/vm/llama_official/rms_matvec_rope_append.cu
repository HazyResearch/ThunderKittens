...
template <typename Config, typename Globals> struct rms_qkv_rope_append {
    ...
    struct pipeline_specifics {
        ...
        load_iter(state<Config> &s, const globals &g, parsed_instruction &inst,
                  int iter, int col_idx, st_bf<16, 512> &weight_chunk,
                  semaphore &sem) {
        ...
        static __device__ inline void store(state<Config> &s, const Globals &g,
            ...
            sv_fl<16> &rope_sin_sv = *reinterpret_cast<sv_fl<16> *>(
            ...
            if (block_idx < V_BLK_START) { // only Q & K need RoPE
                ...
                warp::sync();
                float pair_val =
                    __shfl_sync(MASK_ALL, qkv_proj[0][0], laneid() + mod);
            ...
            if (laneid() == 0) {
                ...
                tma::store_async_wait(); // not just read wait! full wait! must
