...
template <typename Config, typename Globals> struct rms_lm_head {
  static constexpr int opcode =
      OPCODE_RMS_LM_Head; // Op index within the layer -- controls which barrier
                          // to listen to.
  ...
  struct pipeline_specifics {
    ...
    static __device__ inline void store(state<Config> &s, const Globals &g,
      ...
      if (warp::laneid() == 0) {
        ...
        tma::store_async<cache_policy::EVICT_LAST>(g.logits, logits_smem_bf,
                                                   {0, 0, 0, block_idx});
