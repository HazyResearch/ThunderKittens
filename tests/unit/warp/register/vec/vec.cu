...
#else
    std::cout << "Skipping ops/warp/register/vec/conversions tests!" << std::endl;
#endif
#ifdef TEST_WARP_REGISTER_VEC_MAPS
    warp::reg::vec::maps::tests(results);
#else
    std::cout << "Skipping ops/warp/register/vec/maps tests!" << std::endl;
#endif
#ifdef TEST_WARP_REGISTER_VEC_REDUCTIONS
    warp::reg::vec::reductions::tests(results);
#else
    std::cout << "Skipping ops/warp/register/vec/reductions tests!" << std::endl;
#endif
}
