...
struct vec_load_store {
    ...
    template<int S, int NW, kittens::ducks::gl::all GL, kittens::ducks::rt_layout::all L> __device__ static void device_func(const GL &input, GL &output) {
        ...
        kittens::col_vec<kittens::rt_fl<S, S, L>> reg_vec;
...
void warp::memory::vec::shared_to_register::tests(test_data &results) {
    ...
    sweep_gmem_type_1d_warp<vec_load_store, SIZE, kittens::ducks::rt_layout::row>::run(results);
    sweep_gmem_type_1d_warp<vec_load_store, SIZE, kittens::ducks::rt_layout::col>::run(results);
}
