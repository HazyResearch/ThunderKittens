...
template<typename test, int NUM_DEVICES, int MAX_S, int NUM_WORKERS, typename... args>
struct group_vec_p2r_sweep_size_1d_warp_layouts {
    static void run(test_data &results) {    
        group_vec_p2r_sweep_size_1d<test, NUM_DEVICES, MAX_S, NUM_WORKERS, kittens::ducks::rv_layout::ortho>::run(results);
        group_vec_p2r_sweep_size_1d<test, NUM_DEVICES, MAX_S, NUM_WORKERS, kittens::ducks::rv_layout::align>::run(results);
        group_vec_p2r_sweep_size_1d<test, NUM_DEVICES, MAX_S, NUM_WORKERS, kittens::ducks::rv_layout::naive>::run(results);
    }
};
...
struct group_vec_p2r_sweep_size_1d_warp_axes_ops {
    static void run(test_data &results) {    
        ...
        group_vec_p2r_sweep_size_1d_warp_layouts<group_vec_p2r_all_reduce_test<T, kittens::ReduceOp::ADD>, NUM_DEVICES, MAX_S, NUM_WORKERS>::run(results);
        if constexpr (!std::is_same<T, float>::value) {
            group_vec_p2r_sweep_size_1d_warp_layouts<group_vec_p2r_all_reduce_test<T, kittens::ReduceOp::MIN>, NUM_DEVICES, MAX_S, NUM_WORKERS>::run(results);
            group_vec_p2r_sweep_size_1d_warp_layouts<group_vec_p2r_all_reduce_test<T, kittens::ReduceOp::MAX>, NUM_DEVICES, MAX_S, NUM_WORKERS>::run(results);
        }

        group_vec_p2r_sweep_size_1d_warp_layouts<group_vec_p2r_atomic_add_test<T>, NUM_DEVICES, MAX_S, NUM_WORKERS>::run(results);
        group_vec_p2r_sweep_size_1d_warp_layouts<group_vec_p2r_broadcast_test<T>, NUM_DEVICES, MAX_S, NUM_WORKERS>::run(results);
...
void group::memory::vec::pgl_to_register::tests(test_data &results) {
    ...
    if (check_multi_gpus()) {
        group_vec_p2r_sweep_size_1d_warp_axes_ops<float, NUM_GPUS, SIZE, 2>::run(results);
        group_vec_p2r_sweep_size_1d_warp_axes_ops<float, NUM_GPUS, SIZE, 4>::run(results);
        group_vec_p2r_sweep_size_1d_warp_axes_ops<float, NUM_GPUS, SIZE, 12>::run(results);
    
        group_vec_p2r_sweep_size_1d_warp_axes_ops<kittens::bf16, NUM_GPUS, SIZE, 2>::run(results);
        group_vec_p2r_sweep_size_1d_warp_axes_ops<kittens::bf16, NUM_GPUS, SIZE, 4>::run(results);
        group_vec_p2r_sweep_size_1d_warp_axes_ops<kittens::bf16, NUM_GPUS, SIZE, 12>::run(results);
        
        group_vec_p2r_sweep_size_1d_warp_axes_ops<kittens::half, NUM_GPUS, SIZE, 2>::run(results);
        group_vec_p2r_sweep_size_1d_warp_axes_ops<kittens::half, NUM_GPUS, SIZE, 4>::run(results);
        group_vec_p2r_sweep_size_1d_warp_axes_ops<kittens::half, NUM_GPUS, SIZE, 12>::run(results);
    }
}
