...
template<typename test, int NUM_DEVICES, int MAX_H, int MAX_W, int NUM_WORKERS, typename... args>
struct group_p2r_sweep_size_2d_group_axes {
    using I0_t = std::integral_constant<int, 0>;
    using I1_t = std::integral_constant<int, 1>;
    using I2_t = std::integral_constant<int, 2>;

    static void run(test_data &results) {    
        group_p2r_sweep_size_2d<test, NUM_DEVICES, MAX_H, MAX_W, NUM_WORKERS, I2_t>::run(results);
        group_p2r_sweep_size_2d<test, NUM_DEVICES, MAX_H, MAX_W, NUM_WORKERS, I1_t>::run(results);
        group_p2r_sweep_size_2d<test, NUM_DEVICES, MAX_H, MAX_W, NUM_WORKERS, I0_t>::run(results);
    }
};
...
struct group_p2r_sweep_size_2d_group_axes_ops {
    static void run(test_data &results) {    
        ...
        group_p2r_sweep_size_2d_group_axes<group_p2r_all_reduce_test<T, kittens::ReduceOp::ADD>, NUM_DEVICES, MAX_H, MAX_W, NUM_WORKERS>::run(results);
        if constexpr (!std::is_same<T, float>::value) {
            group_p2r_sweep_size_2d_group_axes<group_p2r_all_reduce_test<T, kittens::ReduceOp::MIN>, NUM_DEVICES, MAX_H, MAX_W, NUM_WORKERS>::run(results);
            group_p2r_sweep_size_2d_group_axes<group_p2r_all_reduce_test<T, kittens::ReduceOp::MAX>, NUM_DEVICES, MAX_H, MAX_W, NUM_WORKERS>::run(results);
        }

        group_p2r_sweep_size_2d_group_axes<group_p2r_atomic_add_test<T>, NUM_DEVICES, MAX_H, MAX_W, NUM_WORKERS>::run(results);
        group_p2r_sweep_size_2d_group_axes<group_p2r_broadcast_test<T>, NUM_DEVICES, MAX_H, MAX_W, NUM_WORKERS>::run(results);
...
void group::memory::tile::pgl_to_register::tests(test_data &results) {
    ...
    if (check_multi_gpus()) {
        group_p2r_sweep_size_2d_group_axes_ops<float, NUM_GPUS, SIZE, SIZE, 2>::run(results);
        group_p2r_sweep_size_2d_group_axes_ops<float, NUM_GPUS, SIZE, SIZE, 4>::run(results);
        group_p2r_sweep_size_2d_group_axes_ops<float, NUM_GPUS, SIZE, SIZE, 12>::run(results); // skipped unless INTENSITY = 4
        group_p2r_sweep_size_2d_group_axes_ops<kittens::bf16, NUM_GPUS, SIZE, SIZE, 2>::run(results);
        group_p2r_sweep_size_2d_group_axes_ops<kittens::bf16, NUM_GPUS, SIZE, SIZE, 4>::run(results);
        group_p2r_sweep_size_2d_group_axes_ops<kittens::bf16, NUM_GPUS, SIZE, SIZE, 12>::run(results);
        group_p2r_sweep_size_2d_group_axes_ops<kittens::half, NUM_GPUS, SIZE, SIZE, 2>::run(results);
        group_p2r_sweep_size_2d_group_axes_ops<kittens::half, NUM_GPUS, SIZE, SIZE, 4>::run(results);
        group_p2r_sweep_size_2d_group_axes_ops<kittens::half, NUM_GPUS, SIZE, SIZE, 12>::run(results);
    }
}
