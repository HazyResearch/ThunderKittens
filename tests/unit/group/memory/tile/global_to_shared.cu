...
struct group_shared_load_store {
    ...
    template<int H, int W, int NW> using valid = std::bool_constant<
        (H%NW==0 && W*H<=64) && ( (!std::is_same_v<T, kittens::fp8e4m3> && !std::is_same_v<T, kittens::fp8e5m2>) || W%2 == 0 )
    >;
...
void group::memory::tile::global_to_shared::tests(test_data &results) {
    ...
    sweep_size_2d<group_shared_load_store<kittens::fp8e4m3>, SIZE, SIZE, 2>::run(results);
    sweep_size_2d<group_shared_load_store<kittens::fp8e4m3>, SIZE, SIZE, 4>::run(results);
    sweep_size_2d<group_shared_load_store<kittens::fp8e4m3>, SIZE, 4, 12>::run(results);
    sweep_size_2d<group_shared_load_store<kittens::fp8e5m2>, SIZE, SIZE, 2>::run(results);
    sweep_size_2d<group_shared_load_store<kittens::fp8e5m2>, SIZE, SIZE, 4>::run(results);
    sweep_size_2d<group_shared_load_store<kittens::fp8e5m2>, SIZE, 4, 12>::run(results);
    #endif
}
