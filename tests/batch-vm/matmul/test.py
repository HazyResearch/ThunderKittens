...
# M, K, N = 1536, 16384*15, 1536
# M, K, N = 1664, 16384*15, 1536
# M, K, N = 256, 1024, 256
...
A = (torch.randn((M, K), device=0, dtype=torch.float32) / K**.25).to(torch.bfloat16)
B = (torch.randn((N, K), device=0, dtype=torch.float32) / K**.25).to(torch.bfloat16)
C =  torch.zeros((M, N), device=0, dtype=torch.bfloat16)
...
C2 = (A.to(torch.float16)@B.to(torch.float16).T).to(torch.bfloat16)
