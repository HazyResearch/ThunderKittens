...
struct globals
{
    ...
    using bf16_matrix = gl<bf16, 1, 1, -1, -1, weight_tile, activation_tile, output_tile>;
...
struct MatmulOp {
    ...
    struct parsed_instruction {
        ...
        __device__ inline parsed_instruction(typename config::instruction_t &instruction) {
            row = instruction[1];
            col = instruction[2]; // output block idx, in units of 128 (`128`)
            num_iters = instruction[3]; // iterations on the reduction dimension, in units of 128 (`128`)
        }
        ...
        static __device__ void run(const globals &g, state<config> &s) {
            ...
            if (laneid == 0) { // load B
                ...
                for (int i = 0; i < inst.num_iters; i++) {
                    ...
                    tma::load_async(weight, g.B, {inst.col, i}, inputs_arrived(s, stage));
                }
                ...
                for (int i = 0; i < inst.num_iters; i++) {
                    ...
                    tma::load_async(activation, g.A, {inst.row, i}, inputs_arrived(s, stage));
                }
            }
