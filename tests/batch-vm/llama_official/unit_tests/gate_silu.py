...
gate_weights = torch.randn(NUM_LAYERS, HIDDEN_DIM, HIDDEN_DIM, dtype=torch.bfloat16, device=device)
...
rms_gate_intermediates = torch.randn(BATCH_SIZE, HIDDEN_DIM, dtype=torch.bfloat16, device=device)
...
for batch_start_idx in range(0, BATCH_SIZE // BATCH_BLOCK_SIZE):
    for block_idx in range(0, INTERMEDIATE_DIM // BATCH_BLOCK_SIZE):
        instructions[instruction_idx%SM_COUNT].append([
            GATE_SILU_OPCODE, LAYER_IDX, batch_start_idx, block_idx
