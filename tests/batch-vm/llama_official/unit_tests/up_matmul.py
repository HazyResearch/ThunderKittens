...
from reference import up_matmul
...
up_weights = torch.randn(NUM_LAYERS, INTERMEDIATE_DIM, HIDDEN_DIM, dtype=torch.bfloat16, device=device) * 0.01
...
rms_gate_intermediates = torch.randn(BATCH_SIZE, HIDDEN_DIM, dtype=torch.bfloat16, device=device) * 0.01
...
silu_out = torch.randn(BATCH_SIZE, INTERMEDIATE_DIM, dtype=torch.bfloat16, device=device)
...
for batch_start_idx in range(0, BATCH_SIZE // BATCH_BLOCK_SIZE):
    for block_idx in range(0, INTERMEDIATE_DIM // BATCH_BLOCK_SIZE):
        instructions[instruction_idx%SM_COUNT].append([
            UP_MATMUL_OPCODE, LAYER_IDX, batch_start_idx, block_idx
...
diff = (silu_out - silu_ref).abs()
print(f'silu_out -- max abs diff: {diff.max()}, mean abs diff: {diff.mean()}')
