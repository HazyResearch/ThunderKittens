...
def test_correctness():
    ...
    dt_bias = torch.ones(nheads, dtype=dtype, device=device).requires_grad_()*-4
    ...
    meandiff = torch.mean(torch.abs(y_ref - y_tk))
    close = torch.allclose(y_ref, y_tk, atol=1e-4)
    print(f"TK (BF16) - PyTorch (FP32): Max diff: {diff}; Mean diff: {meandiff}; Close: {close}")

    diff = torch.max(torch.abs(y_ref - y_tk))
    meandiff = torch.mean(torch.abs(y_ref - y_tk))
    close = torch.allclose(y_ref, y_tk, atol=1e-4)
    print(f"TK (BF16) - Triton (FP32): Max diff: {diff}; Mean diff: {meandiff}; Close: {close}")
