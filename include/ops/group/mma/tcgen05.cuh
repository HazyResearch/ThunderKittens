...
template<int trans_a, int n_trans_b, ducks::tt::all D, typename A, ducks::st_descriptor::input B, ducks::tt::all SA, ducks::tt::all SB, int acc=1, int ncta=1, int mma_k_bytes=32, bool a_sf128=false>
__device__ static inline void mma(D &d, const A &a, const B &b, const SA &sa, const SB &sb, semaphore &sem) {
    if(laneid() == 0) ::kittens::mma<trans_a, n_trans_b, D, A, B, SA, SB, acc, ncta, mma_k_bytes, a_sf128>(d, a, b, sa, sb, sem);
}
...
__device__ static inline void mma(D &d, const A &a, const B &b, const SA &sa, const SB &sb) {
    if(laneid() == 0) ::kittens::mma<trans_a, n_trans_b, D, A, B, SA, SB, acc, ncta, mma_k_bytes, a_sf128>(d, a, b, sa, sb);
}
template<int trans_a, int trans_b, ducks::tt::all D, typename A, ducks::st_descriptor::input B, ducks::tt::all SA, ducks::tt::all SB, int acc=1, int mma_k_bytes=32, bool a_sf128=false>
