...
__device__ inline static void load(RV &dst, const SV &_src) {
    ...
    static_assert(SV::length == RV::length*N_WARPS);// confirm size correct
    auto &src = subvec_inplace<RV::length>(_src, warpid()); // pretend it's smaller and do warp-level load
...
__device__ inline static void store(SV &_dst, const RV &src) {
    ...
    static_assert(SV::length == RV::length*N_WARPS);// confirm size correct
    auto &dst = subvec_inplace<RV::length>(_dst, warpid()); // pretend it's smaller and do warp-level load
