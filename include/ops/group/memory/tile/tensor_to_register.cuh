...
__device__ static inline void load_async(RT &dst, const TM &src) {
    ...
    auto src_subtile = src.template subtile<tmem<typename TM::dtype, warp_rows, TM::cols>>(32*(warpid()%4)+16*(warpid()/4), 0);
...
__device__ static inline void store_async(TM &dst, const RT &src) {
    ...
    static_assert(TM::cols==RT::cols);
    static_assert(warp_rows==RT::rows);
    auto dst_subtile = dst.template subtile<tmem<typename TM::dtype, warp_rows, TM::cols>>(32*(warpid()%4)+16*(warpid()/4), 0);
