...
__device__ static inline void atomic_add(const PGL &dst, const ST &src, int dev_id, const COORD &idx) {
    kittens::reduce_op<axis, assume_aligned, ReduceOp::ADD, false, PGL, ST, COORD, GROUP_THREADS>(dst, src, dev_id, idx);
}

template <ducks::pgl::all PGL, ducks::st::all ST, ducks::coord::tile COORD=coord<ST>>
__device__ static inline void atomic_add(const PGL &dst, const ST &src, int dev_id, const COORD &idx) {
    kittens::reduce_op<2, false, ReduceOp::ADD, PGL, ST, COORD, GROUP_THREADS>(dst, src, dev_id, idx);
}
