...
__device__ static inline void prefetch(SV &dst, const GL &src, const COORD &idx) {
    if (::kittens::laneid() == 0) {
        ...
        asm volatile (
            ...
            " [%0, {%1, %2, %3, %4, %5}];"
            :
            : "l"(tma_ptr), "n"(0), "r"(unit_coord.c), "r"(unit_coord.r), "r"(unit_coord.d), "r"(unit_coord.b)
            : "memory"
        );
    }
}
...
__device__ static inline void store_async(const GL &dst, const SV &src, const COORD &idx) {
    if (::kittens::laneid() == 0) {
        ...
        asm volatile (
            ...
            " [%0, {%2, %3, %4, %5, %6}], [%1];"
            :
            : "l"(tma_ptr), "r"(src_ptr), "n"(0), "r"(unit_coord.c), "r"(unit_coord.r), "r"(unit_coord.d), "r"(unit_coord.b)
            : "memory"
        );
    }
    store_commit_group();
...
__device__ static inline void load_async(SV &dst, const GL &src, const COORD &idx, semaphore& bar) {
    if (::kittens::laneid() == 0) {
        ...
        asm volatile (
            ...
            " [%0], [%1, {%3, %4, %5, %6, %7}], [%2];"
            :
            : "r"(dst_ptr), "l"(tma_ptr), "r"(mbar_ptr), "n"(0), "r"(unit_coord.c), "r"(unit_coord.r), "r"(unit_coord.d), "r"(unit_coord.b)
            : "memory"
        );
    }
}
...
__device__ static inline void load_async(SV &dst, const GL &src, const COORD &idx, semaphore& bar, uint16_t cluster_mask) {
    if (::kittens::laneid() == 0) {
        ...
        asm volatile (
            ...
            "n"(0), "r"(unit_coord.c), "r"(unit_coord.r), "r"(unit_coord.d), "r"(unit_coord.b), "h"(cluster_mask)
            : "memory"
        );
    }
}
