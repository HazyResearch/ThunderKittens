...
try:
    from tk_fftconv import TKFFTConv
...
def fftconv_tk_test(dt, b, h, n, dv, verbose=True, **kwargs):
    ...
    conv_tk_4090 = TKFFTConv(n, dtype=u.dtype).to(u.device)

    torch.cuda.synchronize()
    t0 = time.time()

    y_tk = conv_tk_4090(u, k)
...
def apply_rotary_emb_tk(dt, b, h, n, dv):
    ...
    tk.fused_rotary(q, cos, sin, out)
    tk.fused_rotary(k, cos, sin, out)
...
def measure_efficiency(dt, n, method_name, method, verbose=False, use_ones=False, profile=False):
    ...
    # except:
    #     print(f"Exception in {method_name}")
    #     pass
...
if __name__ == "__main__":
    ...
    methods = {
        # based
        # 'based torch': pytorch_test,
        # 'based tk': based_kernel_test,
        # 'based fla': fla_parallel_based_test,
        ...
        # "mamba2": mamba2_kernel_test,

        # hedgehog
        # "hedgehog tk": profile_tk_hedgehog,
        # "hedgehog pytorch": hedgehog_pytorch_test,

        # fft convolution 
        # 'fftconv_cutlass': fftconv_cutlass_test,
        # 'fftconv_tk': fftconv_tk_test,

        # layernorm 
        # 'layernorm torch': pytorch_layernorm_test,
        # 'layernorm triton': triton_layer_norm_test,
        # 'layernorm tk': layer_norm_test,
    ...
    for m, method in methods.items():
        ...
        for n in [
            ...
            measure_efficiency(torch.bfloat16, n, m, method, verbose=False, use_ones=False, profile=False)
        print("\n")
