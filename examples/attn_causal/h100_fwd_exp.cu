...
void attend_ker(int n, int d, const bf16* __restrict__ __q__, const bf16* __restrict__ __k__, const bf16* __restrict__ __v__, bf16* __o__) {
    ...
    st_bf_2x4<ducks::st_layout::swizzle> (&k_smem)[NUM_WORKERS] = al.allocate<st_bf_2x4<ducks::st_layout::swizzle>, NUM_WORKERS>();
    st_bf_2x4<ducks::st_layout::swizzle> (&v_smem)[NUM_WORKERS] = al.allocate<st_bf_2x4<ducks::st_layout::swizzle>, NUM_WORKERS>();

    rt_bf<2, 4> q_reg, k_reg, v_reg;
    rt_fl<2, 2> att_block;
    rt_bf<2, 2> att_block_mma;
    rt_fl<2, 4> o_prev;
    rt_fl<2, 2>::col_vec max_vec_last, max_vec;
    rt_fl<2, 2>::col_vec norm_vec_last, norm_vec;
    ...
    for(auto q_blk = 0; q_blk < qo_blocks; q_blk++) {
        ...
        for(auto kv_idx = 0; kv_idx < kv_blocks; kv_idx++) {
                ...
                subtile++) 
            {
                ...
                rt_bf<2, 4, ducks::rt_layout::col> &v_reg_col = swap_layout_inplace(v_reg); // this is a reference and the call has invalidated v_reg
...
#include "harness_h100_fwd_exp.impl"