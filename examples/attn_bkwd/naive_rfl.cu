...
constexpr int NUM_WORKERS_QO = 4;
...
void attend_ker_bwd(CUtensorMap* tma_q, CUtensorMap* tma_k, CUtensorMap* tma_v, CUtensorMap* tma_o, 
    ...
    st_bf<qo_height, tile_width, layout_q> (&k_smem)   [NUM_WARPGROUPS] = al.allocate<st_bf<qo_height, tile_width, layout_q>, NUM_WARPGROUPS>();
    st_bf<kv_height, tile_width, layout_k> (&k_smem)[2][NUM_WORKERS_QO] = al.allocate<st_bf<kv_height, tile_width, layout_k>, 2, NUM_WORKERS_QO>();
    st_bf<kv_height, tile_width, layout_v> (&v_smem)[2][NUM_WORKERS_QO] = al.allocate<st_bf<kv_height, tile_width, layout_v>, 2, NUM_WORKERS_QO>();
    st_bf<qo_height, tile_width, layout_o> (&v_smem)  [NUM_WARPGROUPS] = al.allocate<st_bf<qo_height, tile_width, layout_o>, NUM_WARPGROUPS>();

    int tic = 0, toc = 1;
 
    rt_fl<1, kv_height> att_block;
    rt_bf<1, kv_height> att_block_mma;
    rt_bf<1, kv_height> att_block_test;
    ...
    constexpr int kv_blocks = N / (NUM_WORKERS_QO*k_smem[0][0].rows);

    __shared__ uint64_t qsmem_barrier, ksmem_barrier, vsmem_barrier, ogsmem_barrier;

    constexpr int tile_bytes = sizeof(bf16) * k_smem[0][0].num_elements * NUM_WORKERS_QO, kPhaseBit = 1;
    ...
    if (warpid == 0) {
        ...
        for (int w = 0; w < NUM_WORKERS_QO; w++) { // load k, v      
            int tile_idx = (blockIdx.y * NUM_WORKERS_QO * kv_blocks) + (0 * NUM_WORKERS_QO) + w; 
    ...
    for(auto kv_idx = 0; kv_idx < kv_blocks; kv_idx++, tic ^= 1, toc ^= 1) {
        ...
        if ((kv_idx + 1 < kv_blocks) && (warpid == 0)) {
            for (int w = 0; w < NUM_WORKERS_QO; w++) {        
                int tile_idx = (blockIdx.y * NUM_WORKERS_QO * kv_blocks) + ((kv_idx + 1) * NUM_WORKERS_QO) + w; 
                tma::load_async((k_smem[toc][w]), tma_k, tile_idx, ksmem_barrier); 
                tma::load_async((v_smem[toc][w]), tma_v, tile_idx, vsmem_barrier); 
            }
        }

        for(int subtile = 0; subtile < NUM_WORKERS_QO; subtile++) {
    ...
    auto *vg_smem = reinterpret_cast<st_bf<qo_height, tile_width, ducks::st_layout::wgmma_col_t_0b>*>(&og_smem[0].data[0]); // reuse og memory
    warpgroup::store(vg_smem[warpgroupid], v_grad);
    __syncthreads();
    if (warpid % 4 == 0) { // store o
        int tile_idx = (blockIdx.y * NUM_WARPGROUPS * blockDim.x) + (blockIdx.x * NUM_WARPGROUPS) + warpgroupid; 
        tma::store_async(tma_v_grad, (vg_smem[warpgroupid]), tile_idx); 
        tma::store_commit_group(); 
    }
    tma::store_async_wait();
}
