...
void attend_ker_bwd(CUtensorMap* tma_q, CUtensorMap* tma_k, CUtensorMap* tma_v, CUtensorMap* tma_og, CUtensorMap* tma_q_grad, CUtensorMap* tma_k_grad, CUtensorMap* tma_v_grad) {
    ...
    auto *temp_g_smem = reinterpret_cast<st_bf<qo_height, tile_width, layout_col>*>(&q_smem[0].data[0]);
    warpgroup::store(temp_g_smem[warpgroupid], q_grad);
    __syncthreads();
    if (warpid == 0) { // store q
        for (int wg = 0; wg < NUM_WORKERS/kittens::WARPGROUP_WARPS; wg++) { // store q_grad
            int tile_idx = (blockIdx.y * NUM_WARPGROUPS * blockDim.x) + (blockIdx.x * NUM_WARPGROUPS) + wg;
            tma::store_async(tma_q_grad, temp_g_smem[wg], tile_idx);
        }
        tma::store_commit_group();
    }
    tma::store_async_wait();

    warpgroup::store(temp_g_smem[warpgroupid], k_grad);
    __syncthreads();
    if (warpid == 0) { // store k
        for (int wg = 0; wg < NUM_WORKERS/kittens::WARPGROUP_WARPS; wg++) { // store k_grad
            int tile_idx = (blockIdx.y * NUM_WARPGROUPS * blockDim.x) + (blockIdx.x * NUM_WARPGROUPS) + wg;
            tma::store_async(tma_k_grad, temp_g_smem[wg], tile_idx);
        }
        tma::store_commit_group();
    }
    tma::store_async_wait();

    warpgroup::store(temp_g_smem[warpgroupid], v_grad);
    __syncthreads();
    if (warpid == 0) { // store v
        for (int wg = 0; wg < NUM_WORKERS/kittens::WARPGROUP_WARPS; wg++) { // store v_grad
            int tile_idx = (blockIdx.y * NUM_WARPGROUPS * blockDim.x) + (blockIdx.x * NUM_WARPGROUPS) + wg;
            tma::store_async(tma_v_grad, temp_g_smem[wg], tile_idx);
        }
        tma::store_commit_group();
    }
    tma::store_async_wait();
