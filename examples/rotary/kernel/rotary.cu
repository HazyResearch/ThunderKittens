...
const int head_dim =  64;                   // TODO: update these dynamically!
...
void fused_rotary( 
    ...
    for (int block = 0; block < n_blocks; block ++, tic ^=1, toc ^=1) {
        ...
        load(cos, cos_s);
        load(sin, sin_s);
        x1.tiles[0][0] = x.tiles[0][0];
        x1.tiles[0][1] = x.tiles[0][1]; // first half dims
        x2.tiles[0][0] = x.tiles[0][2];
        x2.tiles[0][1] = x.tiles[0][3]; // second half dims
        ...
        // assemble the result
        zero(x);
        x.tiles[0][0] = temp1.tiles[0][0];
        x.tiles[0][1] = temp1.tiles[0][1];
        x.tiles[0][2] = temp2.tiles[0][0];
...
fused_rotary_tk(
    ...
    TORCH_CHECK(cos_in.size(1) % kittens::TILE_DIM == 0, "sequence length is divisible by 16?");
    TORCH_CHECK(sin_in.size(1) % kittens::TILE_DIM == 0, "sequence length is divisible by 16?");
    ...
    unsigned long mem_size = N_CHUNK*head_dim*2 + N_CHUNK*half_rope_dim*2*2;
    ...
    fused_rotary<<<batch*n_h,threads,mem_size>>>( 
        n, x_bf, 
        cos_in_bf, sin_in_bf,
        o_bf 
    );
