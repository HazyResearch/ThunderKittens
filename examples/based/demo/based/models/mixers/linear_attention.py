...
try:
    from based.models.mixers.lin_step import lin_attn_step
except:
    lin_attn_step = None
...
class LinearAttention(nn.Module):
    ...
    def parallel_forward(self, x: torch.Tensor, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, impl_choice: str="default"):
        ...
        if impl_choice != "default" and impl_choice == "tk":
            ...
            """
            if not self.add_swish:
                y = fused_chunk_based(q, k, v, use_scale=True, use_normalize=True)
            else:
                y = fused_chunk_based(q, k, v, add_scale=True, use_normalize=False)
