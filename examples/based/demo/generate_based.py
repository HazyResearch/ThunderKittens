...
for input_text in sample_inputs:
    ...
    for model_name, model in zip(['based', 
    # 'mamba', 'attn'
    ], [based_model, 
    # mamba_model, attn_model
    ]):
        model.eval()
        fn = model.generate
        ...
        # after the SWA
        # part_pred = generations[:, start:min(64, generations.shape[1])] 
        # part_pred_ids =  part_pred[0].tolist()
        # part_pred = tokenizer.decode(part_pred_ids)
        print(f"{model_name=}: {input_text} -> {pred}\n")
        # print(start, min(64, generations.shape[1]))
        # print(part_pred)
    print()
