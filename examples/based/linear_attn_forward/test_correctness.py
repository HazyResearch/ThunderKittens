...
def pytorch_test_v4(dt, Q, K, V, d, verbose=True, add_norm=False,  add_scale=False, **kwargs):
    ...
    o, den = 0, 0 
    if add_norm: den += D0.to(torch.float32).unsqueeze(-1)
    o += T0.to(torch.float32)

    if add_norm: den += D1.to(torch.float32).unsqueeze(-1)
    o += T1.to(torch.float32) / (rrd * rrd)
    
    if add_norm: den += D2.to(torch.float32).unsqueeze(-1)
    o += T2.to(torch.float32) / (rd * r2 * rd * r2)

    if add_norm: o = o / (den + eps)
