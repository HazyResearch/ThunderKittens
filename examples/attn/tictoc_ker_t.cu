...
constexpr int NUM_WORKERS_KV_BWD = 4;
constexpr int tile_width_bwd = 64/16;

template<int N> __global__  __launch_bounds__(NUM_WORKERS_BWD*kittens::WARP_THREADS, 1)
void attend_ker_bwd_train(CUtensorMap* tma_q, CUtensorMap* tma_k, CUtensorMap* tma_v, CUtensorMap* tma_o, CUtensorMap* tma_l, 
    ...
    st_bf<kv_height_bwd, tile_width_bwd, layout_k>           (&k_smem)[2][NUM_WORKERS_KV_BWD] = al.allocate<st_bf<kv_height_bwd, tile_width_bwd, layout_k>, 2,       NUM_WORKERS_KV_BWD>();
    st_bf<kv_height_bwd, tile_width_bwd, layout_v>           (&v_smem)[2][NUM_WORKERS_KV_BWD] = al.allocate<st_bf<kv_height_bwd, tile_width_bwd, layout_v>, 2,       NUM_WORKERS_KV_BWD>();
    ...
    constexpr int qo_tiles  = N / q_smem[0].rows; 
    constexpr int kv_blocks = N / (NUM_WORKERS_KV_BWD*k_smem[0][0].rows);

    __shared__ uint64_t qsmem_barrier, ksmem_barrier, vsmem_barrier;

    constexpr int tile_bytes = sizeof(bf16) * k_smem[0][0].num_elements * NUM_WORKERS_KV_BWD; 
    int kPhaseBit = 1;

    if (warpid == 0) {
        tma::init_barrier<st_bf<qo_height_bwd, tile_width_bwd, layout_q>, NUM_WARPGROUPS_BWD>(qsmem_barrier, block.size());
        tma::init_barrier<st_bf<kv_height_bwd, tile_width_bwd, layout_k>, NUM_WORKERS_KV_BWD>(ksmem_barrier, block.size()); 
        tma::init_barrier<st_bf<kv_height_bwd, tile_width_bwd, layout_v>, NUM_WORKERS_KV_BWD>(vsmem_barrier, block.size());
    }
    __syncthreads();
    ...
    if (warpid == 0) {
        ...
        for (int w = 0; w < NUM_WORKERS_KV_BWD; w++) { // load k, v      
            int tile_idx = (blockIdx.y * NUM_WORKERS_KV_BWD * kv_blocks) + (0 * NUM_WORKERS_KV_BWD) + w; 
            tma::load_async((k_smem[tic][w]), tma_k, ksmem_barrier, tile_idx); 
            tma::load_async((v_smem[tic][w]), tma_v, vsmem_barrier, tile_idx); 
        }
    }
    ...
    for(auto kv_idx = 0; kv_idx < kv_blocks; kv_idx++, tic ^= 1, toc ^= 1) {
        ...
        if ((kv_idx + 1 < kv_blocks) && (warpid == 0)) {
            for (int w = 0; w < NUM_WORKERS_KV_BWD; w++) {        
                int tile_idx = (blockIdx.y * NUM_WORKERS_KV_BWD * kv_blocks) + ((kv_idx + 1) * NUM_WORKERS_KV_BWD) + w; 
                tma::load_async((k_smem[toc][w]), tma_k, ksmem_barrier, tile_idx); 
                tma::load_async((v_smem[toc][w]), tma_v, vsmem_barrier, tile_idx); 
            }
        }

        for(int subtile = 0; subtile < NUM_WORKERS_KV_BWD; subtile++) {
            warpgroup::mma_fence(att_block);
            warpgroup::dot_reset(att_block, q_smem[warpgroupid], k_smem[tic][subtile]);
            warpgroup::mma_commit_group();
