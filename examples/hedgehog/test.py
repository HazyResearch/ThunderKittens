...
def main():
    ...
    for seq_len in seq_lens:
        for dtype in dtypes:
            ...
            q = (torch.randn(batch_size, n_heads, seq_len, 16).cuda().to(dtype) / 10).requires_grad_(True)
            k = (torch.randn(batch_size, n_heads, seq_len, 16).cuda().to(dtype) / 10).requires_grad_(True)
            ...
            assert q.allclose(q_tri, 1e-4)
            assert k.allclose(k_tri, 1e-4)
            assert v.allclose(v_tri, 1e-4)
            
            # here, 1e-1 comes from error seen when 
            # we use a fake einsum implementation in pytorch
            if not ref.allclose(tri, 1e-1):
