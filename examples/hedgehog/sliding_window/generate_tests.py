...
if TESTNAME == 'ones':
    ...
    q = torch.eye(D, dtype=torch.bfloat16, device='cuda').reshape((1,1,D,D)).repeat(B, H, N//D, 1)*10
    k = torch.eye(D, dtype=torch.bfloat16, device='cuda').reshape((1,1,D,D)).repeat(B, H, N//D, 1)*10
    v = torch.eye(D, dtype=torch.bfloat16, device='cuda').reshape((1,1,D,D)).repeat(B, H, N//D, 1)*10
...
for t in range(N):
    ...
    if t >= WINDOW:
        att[:,:,t,:t-WINDOW+1] = 0 # make sliding
