...
void kernel(const typename lcft::layout::globals globals,
            uint8_t *dpct_local) {
    auto item_ct1 = sycl::ext::oneapi::this_work_item::get_nd_item<3>();
    static_assert(
        kernel_template<lcft>,
        "lcf kernel template parameter does not satisfy concept requirements");
    ...
    if constexpr (detail::DEBUG_v<lcft>) {
        if (item_ct1.get_local_id(2) == 0 && item_ct1.get_local_id(1) == 0 &&
            ...
            sycl::ext::oneapi::experimental::printf(
                ...
                sizeof(scratch_alloc_block) + uint64_t(&scratch_smem) -
                    uint64_t(&__shm[0]));
        }
        everyone::sync(15);
    }
