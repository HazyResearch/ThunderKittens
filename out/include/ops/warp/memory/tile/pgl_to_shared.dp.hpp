...
static inline void ld_reduce_op(ST &dst, const PGL &src, int dev_idx, const COORD &idx) {
...
template <int axis, bool assume_aligned, ReduceOp OP, ducks::st::all ST, ducks::pgl::all PGL, ducks::coord::tile COORD=coord<ST>, int N_THREADS=WARP_THREADS>
static inline void reduce_op(const PGL &dst, const ST &src, int dev_idx, const COORD &idx) {
    using T = typename ST::dtype;
    using U = typename PGL::dtype;
...
static inline void broadcast(const PGL &dst, const ST &src, int dev_idx, const COORD &idx) {
    ...
    for (int i = 0; i < total_calls; i++) {
        ...
        if constexpr (assume_aligned) {
            ...
            move<sycl::float4>::lds(tmp, src.idx(src_ptr, {row, col}));
            U* dst_ptr = static_cast<U*>(dst_mc_ptr) + row*row_stride + col;
            move<sycl::float4>::stg((sycl::float4 *)dst_ptr, tmp);
        }
        else {
            if (row + unit_coord.template dim<axis>() < dst.template shape<axis>()) {
                sycl::float4 tmp;
