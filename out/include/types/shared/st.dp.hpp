...
struct KITTENS_DEFAULT_ALIGN st {
    ...
    static inline T *idx(T *ptr,
                         sycl::int2 coord) { // naive row-major coord default
    ...
    static inline uint32_t idx(uint32_t ptr, sycl::int2 coord) {
        int r = coord.x(), c = coord.y(); // alias
        static constexpr int swizzle_repeat = swizzle_bytes * 8;
        static constexpr int subtile_cols   = swizzle_bytes / sizeof(T);
        const int outer_idx = c/subtile_cols;
     ...
     */
    inline dtype &operator[](const sycl::int2 &rowcol) {
        return *idx(data, rowcol);
    }
    inline const dtype &operator[](const sycl::int2 &rowcol) const {
...
struct st_subtile {
    ...
    st_subtile(ST &src, sycl::int2 rowcol) {
        data = &src.data[0];
    ...
    inline T *idx(T *ptr,
                  const sycl::int2 coord) { // naive row-major coord default
    ...
    inline uint32_t
    idx(uint32_t ptr,
        const sycl::int2 coord) const { // naive row-major coord default
        int r = coord.x() + row_offset, c = coord.y() + col_offset; // alias
        static constexpr int swizzle_repeat = swizzle_bytes * 8;
        static constexpr int subtile_cols   = swizzle_bytes / sizeof(T);
        const int outer_idx = c/subtile_cols;
    ...
    inline void operator=(const dtype &value) { // runs at warp scope by default
        #pragma unroll
...
using st_hf = st<sycl::half, _height, _width>;
...
inline void print(const ST& tile) {
    if (laneid() == 0) { // Only first thread in warp prints
        printf("Shared Tile %dx%d:\n", ST::rows, ST::cols);
        
        // Print column headers
        sycl::ext::oneapi::experimental::printf(
            "     "); // Padding for row indices
        for (int c = 0; c < ST::cols; c++) {
            sycl::ext::oneapi::experimental::printf("%8d ", c);
        }
        sycl::ext::oneapi::experimental::printf("\n");
        ...
        for (int r = 0; r < ST::rows; r++) {
            sycl::ext::oneapi::experimental::printf("%3d |", r); // Row index
            for (int c = 0; c < ST::cols; c++) {
                if constexpr (std::is_same_v<typename ST::dtype, float>) {
                    ...
                    printf("%8.3f ", sycl::ext::intel::math::bfloat162float(
                                         tile[{r, c}]));
                } else if constexpr (std::is_integral_v<typename ST::dtype>) {
                    sycl::ext::oneapi::experimental::printf("%8d ",
                                                            (int)tile[{r, c}]);
                } else {
                    sycl::ext::oneapi::experimental::printf(
                        "%8.3f ", (float)tile[{r, c}]);
    ...
    sycl::group_barrier(
        sycl::ext::oneapi::this_work_item::get_sub_group()); // Ensure warp
                                                             // stays in sync
}
