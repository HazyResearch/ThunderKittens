...
struct KITTENS_DEFAULT_ALIGN sv {
    ...
    static inline T* idx(T *ptr, int idx) { // useful for computations in shared address space, as silly as it sounds.
        return ptr[idx];
    }

    inline       dtype& operator[](size_t idx)       { return data[idx]; }
    inline const dtype& operator[](size_t idx) const { return data[idx]; }
...
template <size_t _length> using sv_hf = sv<sycl::half, _length>;
template<size_t _length> using sv_fl = sv<float, _length>;

/* ----------  PRINTOUTS  ---------- */

template<ducks::sv::all SV>
inline void print(const SV& sv) {
    if(laneid() == 0) {
        printf("Shared Vector %d:\n", SV::length);
        for(int i = 0; i < SV::length; i++) {
            if constexpr (std::is_same_v<typename SV::dtype, bf16>) {
                printf("%f ", sycl::ext::intel::math::bfloat162float(sv[i]));
            } else if constexpr (std::is_same_v<typename SV::dtype,
                                                sycl::half>) {
                printf("%f ", sycl::ext::intel::math::half2float(sv[i]));
            } else if constexpr (std::is_same_v<typename SV::dtype, float>) {
                printf("%f ", sv[i]);
            } else {
                sycl::ext::oneapi::experimental::printf("%d ", (int)(sv[i]));
            }
        }
