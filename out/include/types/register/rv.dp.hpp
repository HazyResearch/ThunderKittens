...
struct rv {
    ...
    inline       dtype* operator[](size_t idx)       { return &data[idx][0]; } ///< A wrapper for indexing into vector data.
    inline const dtype* operator[](size_t idx) const { return &data[idx][0]; } ///< A wrapper for indexing into vector data.
    inline dtype &operator[](sycl::int2 outin) {
        return data[outin.x()][outin.y()];
    } ///< A wrapper for indexing into vector data.
    inline const dtype &operator[](sycl::int2 outin) const {
        return data[outin.x()][outin.y()];
    } ///< A wrapper for indexing into vector data.
    ...
    template<typename U>
    inline void operator=(const rv<U, length, layout> &other) {
...
using rv_hf = rv<sycl::half, _l, layout>;
