...
class KittensClub {
public:
    inline KittensClub(const int *device_ids, const int num_devices);
    inline ~KittensClub();

    // Dispatches `task` to all threads, and waits for all threads to finish (using cv)
    inline void execute(std::function<void(int)> task);
...
private:
    ...
    inline void worker(int worker_id, int device_id);
...
inline KittensClub::KittensClub(const int *device_ids, const int num_devices) : stop(false), n_task_done(0) {
...
inline KittensClub::~KittensClub() {
    {
...
inline void KittensClub::execute(std::function<void(int)> task) {
    {
        std::lock_guard<std::mutex> lock(mutex);
        current_task = task;
...
inline void KittensClub::worker(int worker_id, int device_id) {
    /*
    DPCT1093:482: The "device_id" device may be not the one intended for use.
