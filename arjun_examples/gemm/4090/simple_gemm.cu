...
template <typename T>
__global__ void simple_gemm(size_t m, size_t n, size_t k, T const* A, T const* B, T* C) {
    ...
    for (auto i = 0; i < K_tiles; i++) {
        kittens::rt_hf_1x1<> a_reg;
        // Load in B as column vector
        kittens::rt<__half2, 1, 1, kittens::ducks::rt_layout::col> b_reg;
        ...
        kittens::mma_AB(c_reg, a_reg, b_reg, c_reg);
    }
    kittens::store(C + (block_row * n) + block_col, c_reg, n);
}

template<typename T>
void launch_simple_gemm(size_t m, size_t n, size_t k, T const* A, T const* B, T* C, cudaStream_t stream){

    // 1 warp handles a single tile
    // Number of threads in each block is num warps times num threads per warp (32)
    const dim3 blockDim = {(unsigned int) kittens::WARP_THREADS};
    ...
    simple_gemm<T><<<gridDim, blockDim>>>(m, n, k, A, B, C);
}
