...
if TESTNAME in ['ones_all']:
    ...
    inp = (torch.ones((BATCH_SIZE, INPUT_LEN, NUM_CHANNELS), dtype=torch.bfloat16, device='cpu')).to(torch.float32) 
    ...
    inp = (torch.randn((BATCH_SIZE, INPUT_LEN, NUM_CHANNELS), dtype=torch.bfloat16, device='cpu')).to(torch.float32)
else:
...
def pytorch_test(kernel, inp, TESTNAME='all'):
    ...
    out = conv(inp.transpose(1, 2)).transpose(1, 2)
...
with open(f'{TESTNAME}_bld.txt', 'w') as f:
